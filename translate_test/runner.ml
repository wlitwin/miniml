...
type test_case = { name : string; source : string; expected : string }
     ...
     while true do
       ...
       if starts_with trimmed "--- test:" then begin
         ...
         | CollectingSource name ->
             ...
             let rest =
               String.trim (String.sub trimmed 11 (String.length trimmed - 11))
             in
             if rest <> "" then begin
       ...
       else if starts_with trimmed "===" then () (* skip section headers *)
       else begin
         match !state with
         | CollectingSource _ ->
...
let run_translator source =
  ...
  let cmd =
    Printf.sprintf "%s %s 2>&1"
      (Filename.quote !translator_path)
...
let () =
  ...
  List.iter
    ...
    files;
  Printf.printf "\n==============================\n";
  Printf.printf "%d/%d translator tests passed" !total_passed
