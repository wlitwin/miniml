    ...
    (fun tc ->
      ...
      | Value _ -> (
          ...
          try
            ...
            in
            if exit_code <> 0 then
              fail tc.name "binary exited with code %d, output: %s" exit_code
                actual
          ...
          | Failure msg -> fail tc.name "codegen failure: %s" msg
          ...
          | exn -> fail tc.name "exception: %s" (Printexc.to_string exn))
      | TypeError -> (
          let oc = open_out tmp_src in
          output_string oc tc.source;
          close_out oc;
          ...
          | Interpreter.Interp.Error msg
            when String.length msg > 10 && String.sub msg 0 10 = "Type error" ->
              ...
              incr passed
          | exn ->
              fail tc.name "expected type error, got: %s"
      ...
      | TypeErrorMsg substr -> (
          ...
          | Interpreter.Typechecker.Type_error (msg, _) ->
              if contains_substring msg substr then begin
                ...
                fail tc.name "expected type error containing %S, got: %s" substr
                  msg
      ...
      | RuntimeError substr -> (
          ...
          try
            ...
            if exit_code <> 0 && contains_substring output substr then begin
              ...
              fail tc.name "expected error containing %S, got: %s" substr
                (String.trim output)
          with
          | Failure msg
