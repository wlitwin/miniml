...
let () =
  ...
  test "basic class and instance" (fun () -> expect_string "show 42" "42");
  ...
  test "show float instance" (fun () -> expect_string "show 3.14" "3.14");
  ...
  (* Unified + for float *)
  test "float addition with +" (fun () -> expect_float {|2.0 + 3.0|} 5.0);

  test "float subtraction with -" (fun () -> expect_float {|10.0 - 4.0|} 6.0);

  test "float multiplication with *" (fun () -> expect_float {|3.0 * 2.0|} 6.0);

  test "float division with /" (fun () -> expect_float {|10.0 / 4.0|} 2.5);

  test "float negation" (fun () -> expect_float {|-(3.5)|} (-3.5));

  (* Bitwise operators *)
  test "bitwise land" (fun () -> expect_int {|7 land 3|} 3);

  test "bitwise lor" (fun () -> expect_int {|5 lor 3|} 7);

  test "bitwise lxor" (fun () -> expect_int {|5 lxor 3|} 6);

  test "bitwise lsl" (fun () -> expect_int {|1 lsl 3|} 8);

  test "bitwise lsr" (fun () -> expect_int {|8 lsr 2|} 2);

  test "bitwise lnot" (fun () -> expect_int {|lnot 0|} (-1));

  (* First-class operators through typeclasses *)
  test "first-class (+) int" (fun () -> expect_int {|let f = (+) in f 3 4|} 7);

  test "first-class (*) int" (fun () -> expect_int {|let f = (*) in f 5 6|} 30);
  ...
  test "first-class (land)" (fun () -> expect_int {|let f = (land) in f 7 3|} 3);

  (* fold with (+) *)
  test "fold with (+)" (fun () -> expect_int {|fold (+) 0 [1; 2; 3; 4]|} 10);

  (* String comparison *)
  test "string less than" (fun () -> expect_bool {|"abc" < "def"|} true);

  test "string greater than" (fun () -> expect_bool {|"xyz" > "abc"|} true);

  test "string le equal" (fun () -> expect_bool {|"abc" <= "abc"|} true);

  test "string ge" (fun () -> expect_bool {|"abc" >= "abd"|} false);
  ...
  test "structural inequality" (fun () -> expect_bool {|[1; 2] <> [1; 3]|} true);
  ...
  test "module class browse" (fun () ->
    ...
    |}
      in
      let state =
        match !((get_stdlib_state ()).Interpreter.Interp.state_ref) with
        | Some s -> s
  ...
  test "show int pair" (fun () -> expect_stdlib_string {|show (1, 2)|} "(1, 2)");
