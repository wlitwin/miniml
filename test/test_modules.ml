...
let () =
  ...
  test "Stdlib.mod works" (fun () -> expect_int {|Stdlib.mod 10 3|} 1);
  ...
  test "basic pipe" (fun () -> expect_int {|1 |> fn x -> x + 1|} 2);
  ...
  test "format float .2f" (fun () -> expect_string {|$"{3.14159:.2f}"|} "3.14");

  test "format float .0f" (fun () -> expect_string {|$"{3.14159:.0f}"|} "3");

  test "format float .4f" (fun () -> expect_string {|$"{2.5:.4f}"|} "2.5000");

  test "format hex lowercase" (fun () -> expect_string {|$"{255:x}"|} "ff");

  test "format hex uppercase" (fun () -> expect_string {|$"{255:X}"|} "FF");

  test "format octal" (fun () -> expect_string {|$"{8:o}"|} "10");

  test "format binary" (fun () -> expect_string {|$"{10:b}"|} "1010");

  test "format binary zero" (fun () -> expect_string {|$"{0:b}"|} "0");
