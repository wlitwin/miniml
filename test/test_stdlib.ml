...
let () =
  ...
  test "String.replace" (fun () ->
      expect_stdlib_string {| String.replace "world" "ocaml" "hello world" |}
        "hello ocaml");

  test "String.to_int Some" (fun () ->
      expect_stdlib_value {| String.to_int "42" |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VInt 42))));
  ...
  test "String.to_float Some" (fun () ->
      expect_stdlib_value {| String.to_float "3.14" |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VFloat 3.14))));
  ...
  test "List.length" (fun () -> expect_stdlib_int {| List.length [1; 2; 3] |} 3);

  test "List.length empty" (fun () -> expect_stdlib_int {| List.length [] |} 0);

  test "List.rev" (fun () ->
      expect_stdlib_value {| List.rev [1; 2; 3] |}
        (Interpreter.Bytecode.VList [ VInt 3; VInt 2; VInt 1 ]));

  test "List.hd" (fun () -> expect_stdlib_int {| List.hd [10; 20; 30] |} 10);

  test "List.tl" (fun () ->
      expect_stdlib_value {| List.tl [1; 2; 3] |}
        (Interpreter.Bytecode.VList [ VInt 2; VInt 3 ]));

  test "List.nth" (fun () -> expect_stdlib_int {| List.nth [10; 20; 30] 1 |} 20);
  ...
  test "List.find Some" (fun () ->
      expect_stdlib_value {| List.find (fn x -> x > 2) [1; 2; 3; 4] |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VInt 3))));
  ...
  test "Math.abs positive" (fun () -> expect_stdlib_int {| Math.abs 5 |} 5);

  test "Math.abs negative" (fun () ->
      expect_stdlib_int {| Math.abs (0 - 5) |} 5);

  test "Math.min" (fun () -> expect_stdlib_int {| Math.min 3 7 |} 3);

  test "Math.max" (fun () -> expect_stdlib_int {| Math.max 3 7 |} 7);

  test "Math.pow" (fun () -> expect_stdlib_float {| Math.pow 2.0 10.0 |} 1024.0);

  test "Math.sqrt" (fun () -> expect_stdlib_float {| Math.sqrt 16.0 |} 4.0);

  test "Math.floor" (fun () -> expect_stdlib_int {| Math.floor 3.7 |} 3);

  test "Math.ceil" (fun () -> expect_stdlib_int {| Math.ceil 3.2 |} 4);

  test "Math.round" (fun () -> expect_stdlib_int {| Math.round 3.5 |} 4);

  test "Math.round down" (fun () -> expect_stdlib_int {| Math.round 3.4 |} 3);
  ...
  test "IO.write and read file" (fun () ->
      let _ =
        run_stdlib {| IO.write_file "/tmp/_miniml_test.txt" "hello stdlib" |}
      in
      expect_stdlib_string {| IO.read_file "/tmp/_miniml_test.txt" |}
        "hello stdlib");
  ...
  test "Sys.time returns float" (fun () ->
      ...
      | v ->
          failwith
            (Printf.sprintf "expected positive float, got %s"
  ...
  test "Result Ok" (fun () ->
      expect_stdlib_value {| Ok 42 |}
        (Interpreter.Bytecode.VVariant
           (0, "Ok", Some (Interpreter.Bytecode.VInt 42))));

  test "Result Err" (fun () ->
      expect_stdlib_value {| Err "fail" |}
        (Interpreter.Bytecode.VVariant
           (1, "Err", Some (Interpreter.Bytecode.VString "fail"))));
  ...
  test "Result.map Ok" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VVariant
           (0, "Ok", Some (Interpreter.Bytecode.VInt 11))));
  ...
  test "Result.map Err" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VVariant
           (1, "Err", Some (Interpreter.Bytecode.VString "bad"))));
  ...
  test "Runtime.eval_file" (fun () ->
      let _ =
        run_stdlib
          {| IO.write_file "/tmp/_miniml_eval_test.ml" "let eval_loaded = 99" |}
      in
      expect_stdlib_value
        {|
      ...
      let _ =
        run_stdlib
          {| IO.write_file "/tmp/_miniml_mod_test.ml"
           "module TestMod = pub let add x y = x + y end" |}
      in
      let state = get_stdlib_state () in
      let state2, _, _ =
        Interpreter.Interp.eval_repl state
          {| Runtime.eval_file "/tmp/_miniml_mod_test.ml" |}
      in
      let _, result, _ =
        Interpreter.Interp.eval_repl state2 {| TestMod.add 3 4 |}
      in
  ...
  test "Runtime.eval module available in REPL" (fun () ->
      ...
      let state2, _, _ =
        Interpreter.Interp.eval_repl state
          {| Runtime.eval "module DynMod = pub let double x = x * 2 end" |}
      in
      let _, result, _ =
        Interpreter.Interp.eval_repl state2 {| DynMod.double 21 |}
      in
  ...
  test "byte of_int to_int" (fun () ->
      expect_stdlib_int {|Byte.to_int (Byte.of_int 97)|} 97);
  test "byte to_string" (fun () ->
      expect_stdlib_string {|Byte.to_string #41|} "A");
  test "byte is_alpha true" (fun () ->
      expect_stdlib_bool {|Byte.is_alpha #41|} true);
  test "byte is_alpha false" (fun () ->
      expect_stdlib_bool {|Byte.is_alpha #30|} false);
  test "byte is_digit true" (fun () ->
      expect_stdlib_bool {|Byte.is_digit #39|} true);
  test "byte is_digit false" (fun () ->
      expect_stdlib_bool {|Byte.is_digit #41|} false);
  test "byte is_space true" (fun () ->
      expect_stdlib_bool {|Byte.is_space #20|} true);
  test "byte is_upper true" (fun () ->
      expect_stdlib_bool {|Byte.is_upper #41|} true);
  test "byte is_lower true" (fun () ->
      expect_stdlib_bool {|Byte.is_lower #61|} true);
  test "byte to_upper" (fun () ->
      expect_stdlib_int {|Byte.to_int (Byte.to_upper #61)|} 65);
  test "byte to_lower" (fun () ->
      expect_stdlib_int {|Byte.to_int (Byte.to_lower #41)|} 97);
  ...
  test "rune of_int to_int" (fun () ->
      expect_stdlib_int {|Rune.to_int (Rune.of_int 65)|} 65);
  test "rune to_string" (fun () ->
      expect_stdlib_string {|Rune.to_string 'a'|} "a");
  test "rune escape newline" (fun () ->
      expect_stdlib_int {|Rune.to_int '\n'|} 10);
  test "rune escape tab" (fun () -> expect_stdlib_int {|Rune.to_int '\t'|} 9);
  test "rune escape null" (fun () -> expect_stdlib_int {|Rune.to_int '\0'|} 0);
  test "rune is_alpha true" (fun () ->
      expect_stdlib_bool {|Rune.is_alpha 'a'|} true);
  test "rune is_digit true" (fun () ->
      expect_stdlib_bool {|Rune.is_digit '5'|} true);
  test "rune is_space true" (fun () ->
      expect_stdlib_bool {|Rune.is_space ' '|} true);
  test "rune is_upper true" (fun () ->
      expect_stdlib_bool {|Rune.is_upper 'A'|} true);
  test "rune is_lower true" (fun () ->
      expect_stdlib_bool {|Rune.is_lower 'a'|} true);
  ...
  test "Set.empty" (fun () -> expect_stdlib_int {|Set.size (Set.empty ())|} 0);
  ...
  test "Set.union" (fun () ->
      expect_stdlib_int
        {|Set.size (Set.union (Set.of_list [1; 2]) (Set.of_list [2; 3]))|} 3);
  test "Set.inter" (fun () ->
      expect_stdlib_int
  ...
  test "Set.is_subset true" (fun () ->
      expect_stdlib_bool
        {|Set.is_subset (Set.of_list [1; 2]) (Set.of_list [1; 2; 3])|} true);
  test "Set.is_subset false" (fun () ->
      expect_stdlib_bool
        {|Set.is_subset (Set.of_list [1; 4]) (Set.of_list [1; 2; 3])|} false);
  ...
  test "Set.for_loop_print" (fun () ->
      expect_stdlib_value {|for x in Set.of_list [10; 20; 30] do print x end|}
        Interpreter.Bytecode.VUnit);

  Printf.printf "\n=== Enum Tests ===\n";

  test "Enum.sum" (fun () -> expect_stdlib_int {|Enum.sum [1; 2; 3; 4; 5]|} 15);
  test "Enum.count" (fun () ->
  ...
  test "Enum.enumerate" (fun () ->
      expect_stdlib_value {|Enum.enumerate ["a"; "b"; "c"]|}
        (VList
           [
             VTuple [| VInt 0; VString "a" |];
             VTuple [| VInt 1; VString "b" |];
             VTuple [| VInt 2; VString "c" |];
           ]));
  test "Enum.join" (fun () ->
  ...
  test "Enum.min_by" (fun () ->
      expect_stdlib_value {|Enum.min_by (fn x -> x) [3; 1; 4; 1; 5]|} (VInt 1));
  test "Enum.max_by" (fun () ->
      expect_stdlib_value {|Enum.max_by (fn x -> x) [3; 1; 4; 1; 5]|} (VInt 5));
  ...
  test "Seq.count" (fun () -> expect_stdlib_int {|Seq.count (Seq.range 1 6)|} 5);
  test "Seq.sum" (fun () -> expect_stdlib_int {|Seq.sum (Seq.range 1 6)|} 15);
  ...
  test "Dynarray to_array" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VArray
           [| Interpreter.Bytecode.VInt 10; Interpreter.Bytecode.VInt 20 |]));
  ...
  test "List.find_map found" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VInt 40))));
  ...
  test "List.assoc_opt found" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VInt 2))));
  ...
  test "List.map2" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VList
           [
             Interpreter.Bytecode.VInt 11;
             Interpreter.Bytecode.VInt 22;
  ...
  test "String.rindex_opt" (fun () ->
    ...
    |}
        (Interpreter.Bytecode.VVariant
           (1, "Some", Some (Interpreter.Bytecode.VInt 7))));
