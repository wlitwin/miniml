...
let () =
  ...
  test "lex integer" (fun () ->
      ...
      match tokens with
      | [
       { kind = Interpreter.Token.INT 42; _ };
       { kind = Interpreter.Token.EOF; _ };
      ] ->
          ()
  ...
  test "lex keywords" (fun () ->
      let tokens =
        Interpreter.Lexer.tokenize "let rec in if else fn match with type of"
      in
      let kinds =
        List.map (fun (t : Interpreter.Token.token) -> t.kind) tokens
      in
      match kinds with
  ...
  test "short-circuit or" (fun () -> expect_bool "true || (1 = 0)" true);
  ...
  test "nested let" (fun () -> expect_int "let x = 5 in let y = 10 in x + y" 15);
  test "let shadowing" (fun () -> expect_int "let x = 5 in let x = 10 in x" 10);
  ...
  test "higher-order function" (fun () ->
      expect_int
        "let apply (f: int -> int) (x: int) : int = f x in apply (fn (x: int) \
         -> x * 2) 5"
        10);
  test "closure" (fun () ->
      expect_int
        "let make_adder (n: int) : int -> int = fn (x: int) -> x + n in let \
         add5 = make_adder 5 in add5 10"
        15);
  ...
  test "factorial" (fun () ->
      expect_int
        "let rec fact (n: int) : int = if n <= 1 do 1 else n * fact (n - 1) in \
         fact 10"
        3628800);
  test "fibonacci" (fun () ->
      expect_int
        "let rec fib (n: int) : int = if n <= 1 do n else fib (n-1) + fib \
         (n-2) in fib 10"
        55);
  ...
  test "nested if" (fun () ->
      expect_int "if true do if false do 1 else 2 else 3" 2);
  ...
  test "sequence" (fun () -> expect_int "1; 2; 3" 3);

  Printf.printf "\n=== Print Tests ===\n";

  test "print returns unit" (fun () -> expect_unit "print 42");

  Printf.printf "\n=== Type Error Tests ===\n";

  test "add int to bool" (fun () -> expect_type_error "1 + true");
  test "if non-bool condition" (fun () -> expect_type_error "if 1 do 2 else 3");
  ...
  test "plus as value" (fun () -> expect_int "let f = (+) in f 3 4" 7);
  ...
  test "comparison as value" (fun () -> expect_bool "let f = (<) in f 3 5" true);
  ...
  test "int_of_float" (fun () -> expect_int "int_of_float 3.14" 3);
  test "not as value" (fun () -> expect_bool "let f = (not) in f true" false);
  ...
  test "raw string basic" (fun () -> expect_string "{|hello|}" "hello");

  test "raw string empty" (fun () -> expect_string "{||}" "");
  ...
  test "print still works" (fun () -> expect_unit "print 42");
  ...
  test "float add with polymorphic +" (fun () -> expect_float "1.0 + 2.0" 3.0);

  test "float sub with polymorphic -" (fun () -> expect_float "5.0 - 1.5" 3.5);

  test "float mul with polymorphic *" (fun () -> expect_float "2.0 * 3.0" 6.0);

  test "float div with polymorphic /" (fun () -> expect_float "10.0 / 4.0" 2.5);

  test "float negation" (fun () -> expect_float "-(3.5)" (-3.5));

  Printf.printf "\n=== Do/End Tests ===\n";

  test "do end basic" (fun () -> expect_int {| do 42 end |} 42);
