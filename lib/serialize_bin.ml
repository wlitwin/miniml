...
type string_table = { tbl : (string, int) Hashtbl.t; mutable count : int }
            ...
            Array.iteri
              (fun i v ->
                ...
                | Bytecode.VExternal ext ->
                    ignore (strtab_intern st ext.ext_name)
                | _ -> ())
              values
      | _ -> ())
    globals
...
let write_captures buf caps =
  ...
  List.iter
    (fun cap ->
      match cap with
      | Bytecode.CaptureLocal i ->
          write_u8 buf 0;
          write_u32 buf i
      | Bytecode.CaptureUpvalue i ->
          write_u8 buf 1;
          write_u32 buf i)
    caps
...
let write_opcode buf st = function
  ...
  | Bytecode.JUMP off ->
      write_u8 buf 35;
      write_u32 buf off
  | Bytecode.JUMP_IF_FALSE off ->
      write_u8 buf 36;
      write_u32 buf off
  | Bytecode.JUMP_IF_TRUE off ->
      write_u8 buf 37;
      write_u32 buf off
  | Bytecode.CLOSURE (i, caps) ->
      write_u8 buf 38;
      write_u32 buf i;
      write_captures buf caps
  ...
  | Bytecode.MATCH_FAIL loc ->
      write_u8 buf 60;
      write_u32 buf (strtab_intern st loc)
  | Bytecode.PERFORM op ->
      write_u8 buf 61;
  ...
  | Bytecode.ENTER_LOOP n ->
      write_u8 buf 64;
      write_u32 buf n
  | Bytecode.EXIT_LOOP -> write_u8 buf 65
  | Bytecode.LOOP_BREAK -> write_u8 buf 66
  | Bytecode.LOOP_CONTINUE n ->
      write_u8 buf 67;
      write_u32 buf n
  | Bytecode.FOLD_CONTINUE n ->
      write_u8 buf 68;
      write_u32 buf n
...
let rec write_value buf st = function
  ...
  | Bytecode.VBool b ->
      write_u8 buf 2;
      write_u8 buf (if b then 1 else 0)
  | Bytecode.VString s ->
      write_u8 buf 3;
      write_u32 buf (strtab_intern st s)
  | Bytecode.VByte n ->
      write_u8 buf 4;
      write_u8 buf n
  | Bytecode.VRune n ->
          ...
          if all_external && Array.length values > 0 then begin
            let fs =
              List.init (Array.length shape.rs_fields) (fun i ->
                  match values.(i) with
                  | Bytecode.VExternal ext ->
                      (shape.rs_fields.(i), ext.ext_name, ext.ext_arity)
                  | _ -> assert false)
            in
  ...
  List.iter
    (fun (idx, entry) ->
      ...
      | `Dict fields ->
          ...
          write_u32 buf (List.length fields);
          List.iter
            (fun (field_name, ext_name, ext_arity) ->
...
let convert_json_to_binary json_str =
  ...
  let global_name_list =
    List.map json_string (json_list (json_field "global_names" root))
  in
      ...
      List.iter
        (fun (idx_str, entry) ->
          ...
          | "external" ->
              ...
              native_entries :=
                (idx, `External (name, arity)) :: !native_entries
          | "dict" ->
              let field_list =
                match json_field "fields" entry with
                | JObject fl -> fl
                | _ -> []
              in
              let fs =
                List.map
                  (fun (field_name, field_entry) ->
  ...
  List.iter
    (fun (_, entry) ->
      ...
      | `Dict fs ->
          List.iter
            (fun (fn, en, _) ->
              ignore (strtab_intern st fn);
              ignore (strtab_intern st en))
            fs)
    native_entries;
  ...
  Array.iter
    (fun name -> write_u32 buf (strtab_intern st name))
    global_names_arr;
  (* Native globals *)
