...
type stats = {
  ...
  mutable const_fold_bool: int;     (* TEBinop on two TEBool; TEUnop Not on TEBool *)
  ...
  mutable known_ctor_match: int;    (* TEMatch on TEConstruct — statically resolvable *)
  ...
  mutable match_on_let: int;        (* match (let x = e1 in e2) — inline could expose known ctor *)
  mutable if_on_app: int;           (* if f(...) — function inlining could make const-foldable *)
  mutable total_apps: int;          (* total function applications *)
  mutable app_of_var: int;          (* f(x) where f is a variable — potential inline target *)
...
let is_identity_op te =
  ...
  | TEBinop (Ast.Concat, l, r) ->
    (match l.expr, r.expr with TEString "", _ | _, TEString "" -> true | _ -> false)
  | TEBinop (Ast.Or, l, r) ->
    (match l.expr, r.expr with TEBool false, _ | _, TEBool false -> true | _ -> false)
  | TEBinop (Ast.And, l, r) ->
    (match l.expr, r.expr with TEBool true, _ | _, TEBool true -> true | _ -> false)
  | _ -> false
...
let analyze_expr stats te =
  let rec go te =
    ...
    (match te.expr with
     | TEBinop (_, l, r) ->
       (match l.expr, r.expr with
        ...
        | TEFloat _, TEFloat _ -> stats.const_fold_float <- stats.const_fold_float + 1
        | TEBool _, TEBool _ -> stats.const_fold_bool <- stats.const_fold_bool + 1
        | TEString _, TEString _ -> stats.const_fold_string <- stats.const_fold_string + 1
        | _ -> ())
     | TEUnop (Ast.Not, e) ->
       (match e.expr with TEBool _ -> stats.const_fold_bool <- stats.const_fold_bool + 1 | _ -> ())
     | TEUnop (Ast.Neg, e) ->
       (match e.expr with
    ...
    (match te.expr with
     ...
     | _ -> ());
    (* Identity ops *)
    if is_identity_op te then
      stats.identity_ops <- stats.identity_ops + 1;
     ...
     | TELet (name, _, rhs, body) ->
       ...
       if uses = 1 then begin
         ...
         (match rhs.expr with
          ...
          | _ when is_literal rhs -> stats.single_use_literal <- stats.single_use_literal + 1
          | _ ->
            let sz = expr_size rhs in
            if sz <= 5 then stats.single_use_small <- stats.single_use_small + 1
    ...
    (match te.expr with
     | TEFun (_, body, _) ->
       stats.fun_sizes <- expr_size body :: stats.fun_sizes
     | _ -> ());
    (* Let-in-let RHS *)
    (match te.expr with
     | TELet (_, _, rhs, _) ->
       (match rhs.expr with TELet _ | TELetRec _ -> stats.let_in_let_rhs <- stats.let_in_let_rhs + 1 | _ -> ())
     | _ -> ());
    ...
    (match te.expr with
     | TEFun (param, { expr = TEApp (_, { expr = TEVar x; _ }); _ }, _) when String.equal param x ->
    ...
    (match te.expr with
     | TEApp _ ->
       ...
       let rec get_head e = match e.expr with
...
let rec analyze_decl stats decl =
  ...
  | TDLetRecAnd (binds) ->
    List.iter (fun (_, e) -> analyze_expr stats e) binds
  | TDModule (_, decls, _) ->
    List.iter (analyze_decl stats) decls
...
let print_stats stats =
  ...
  let total_const = stats.const_fold_int + stats.const_fold_float +
    stats.const_fold_bool + stats.const_fold_string +
    stats.const_fold_if + stats.identity_ops in
  Printf.printf "  TOTAL constant fold:           %d (%.1f%% of nodes)\n\n"
    total_const (100.0 *. float_of_int total_const /. float_of_int (max 1 stats.total_nodes));
  ...
  Printf.printf "  Dead let (0 uses in body):     %d (%.1f%% of lets)\n"
    stats.dead_let (100.0 *. float_of_int stats.dead_let /. float_of_int (max 1 stats.total_lets));
  if stats.dead_let_names <> [] then begin
    let names = List.rev stats.dead_let_names in
    let shown = if List.length names > 20 then List.filteri (fun i _ -> i < 20) names @ ["..."] else names in
  ...
  Printf.printf "  Single-use let bindings:       %d (%.1f%% of lets)\n"
    stats.single_use_let (100.0 *. float_of_int stats.single_use_let /. float_of_int (max 1 stats.total_lets));
  ...
  let total_known = stats.known_ctor_match + stats.known_tuple_match + stats.known_bool_match in
  ...
  if n > 0 then begin
    ...
    let large = List.length (List.filter (fun s -> s > 20 && s <= 100) sorted) in
    let huge = List.length (List.filter (fun s -> s > 100) sorted) in
    Printf.printf "  Tiny (<=5 nodes):              %d (%.0f%%) — trivially inlinable\n"
      small (100.0 *. float_of_int small /. float_of_int n);
    Printf.printf "  Small (6-20 nodes):            %d (%.0f%%) — likely inlinable\n"
      medium (100.0 *. float_of_int medium /. float_of_int n);
    Printf.printf "  Medium (21-100 nodes):         %d (%.0f%%)\n"
      large (100.0 *. float_of_int large /. float_of_int n);
    Printf.printf "  Large (>100 nodes):            %d (%.0f%%)\n"
      huge (100.0 *. float_of_int huge /. float_of_int n)
  end;

  Printf.printf "--- Application Analysis ---\n";
  Printf.printf "  Total applications:            %d\n" stats.total_apps;
  Printf.printf "  Direct calls (f x):            %d (%.0f%%)\n"
    stats.app_of_var (100.0 *. float_of_int stats.app_of_var /. float_of_int (max 1 stats.total_apps));
  Printf.printf "  Method calls (obj.m x):        %d (%.0f%%)\n"
    stats.app_of_field (100.0 *. float_of_int stats.app_of_field /. float_of_int (max 1 stats.total_apps));
  ...
  Printf.printf "  match on let-bound value:      %d (inlining let could expose known ctor)\n" stats.match_on_let;
  Printf.printf "  if on function call:           %d (inlining callee could make const-foldable)\n\n" stats.if_on_app;
  ...
  Printf.printf "  Single-use + small body lets:  ~%d nodes eliminable via inlining\n" inlinable_single;
  ...
  Printf.printf "  Beta reduction:                ~%d function applications eliminable\n" stats.beta_reducible;
  let total_reducible = inlinable_single + stats.dead_let + total_const + total_known + stats.beta_reducible in
  Printf.printf "  TOTAL estimated reductions:    ~%d (%.1f%% of %d nodes)\n"
    total_reducible
    (100.0 *. float_of_int total_reducible /. float_of_int (max 1 stats.total_nodes))
    stats.total_nodes
