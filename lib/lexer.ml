...
let make_token l kind loc = Token.{ kind; loc; end_offset = l.cursor }
let at_end l = l.cursor >= l.len
let peek l = if at_end l then '\000' else l.source.[l.cursor]
let peek2 l = if l.cursor + 1 >= l.len then '\000' else l.source.[l.cursor + 1]
let peek3 l = if l.cursor + 2 >= l.len then '\000' else l.source.[l.cursor + 2]
...
let error l msg = raise (Lex_error (msg, current_loc l))
...
let skip_comment l =
  ...
  while !depth > 0 do
    ...
    else if peek l = '*' && peek2 l = ')' then begin
      ignore (advance l);
      ignore (advance l);
      depth := !depth - 1
    end
    else ignore (advance l)
  done
...
let read_number l =
  ...
  if c = '0' && (not (at_end l)) && (peek l = 'x' || peek l = 'X') then begin
    ...
    let n =
      ...
      | Some n -> n
      | None -> raise (Lex_error ("integer literal overflow", loc))
    in
    make_token l (INT n) loc
  end
  else if c = '0' && (not (at_end l)) && (peek l = 'b' || peek l = 'B') then begin
    Buffer.add_char buf (advance l);
    ...
    if (not (at_end l)) && peek l = '.' then begin
      ...
      let f =
        ...
        | Some f -> f
        | None -> raise (Lex_error ("float literal overflow", loc))
      in
...
let read_string l =
  ...
  while (not (at_end l)) && peek l <> '"' do
    ...
    else Buffer.add_char buf (advance l)
  done;
  if at_end l then error l "unterminated string";
  ignore (advance l);
  (* consume closing '"' *)
...
let dedent_raw s =
    ...
    List.fold_left
      (fun acc line ->
        if String.trim line = "" then acc
          ...
          match acc with None -> Some !i | Some prev -> Some (min prev !i))
      None lines
  in
  ...
  let lines =
    List.map
      (fun line ->
        let len = String.length line in
        if len >= indent then String.sub line indent (len - indent) else line)
      lines
  in
  String.concat "\n" lines
  ...
  while (not (at_end l)) && peek l <> '"' do
    if peek l = '\\' then begin
      ...
      while !depth > 0 do
        ...
        else if c = ':' && !depth = 1 && !paren_depth = 0 && not !in_fmt then begin
          ignore (advance l);
          (* consume ':' *)
          in_fmt := true
        end
        ...
        else Buffer.add_char (if !in_fmt then fmt_buf else expr_buf) (advance l)
      done;
...
let read_ident_or_keyword l =
  ...
  let kind =
    ...
    | None ->
        ...
        else if Char.uppercase_ascii s.[0] = s.[0] && s.[0] <> '_' then
          Token.UIDENT s
        else Token.IDENT s
  in
...
let rec next_token l =
  ...
  if at_end l then make_token l EOF (current_loc l)
  else
    let loc = current_loc l in
    let c = peek l in
    match c with
    ...
    | '\'' ->
        ...
        in
        if at_end l || peek l <> '\'' then error l "unterminated rune literal";
        ignore (advance l);
    ...
    | '-' ->
        ...
        else if (not (at_end l)) && peek l = '>' then begin
          ignore (advance l);
          make_token l ARROW loc
        end
        else make_token l MINUS loc
    | ';' ->
        ignore (advance l);
        if (not (at_end l)) && peek l = ';' then begin
          ignore (advance l);
        ...
        else make_token l SEMICOLON loc
    | ':' ->
        ignore (advance l);
        if (not (at_end l)) && peek l = '=' then begin
          ignore (advance l);
    ...
    | '&' ->
        ...
        else error l "unexpected '&', did you mean '&&'?"
    | '|' ->
        ignore (advance l);
        if (not (at_end l)) && peek l = '|' then begin
          ignore (advance l);
    ...
    | '`' ->
        ...
        if (not (at_end l)) && peek l >= 'A' && peek l <= 'Z' then begin
          ...
          (not (at_end l)) && ((peek l >= 'a' && peek l <= 'z') || peek l = '_')
        then begin
          let buf = Buffer.create 16 in
          while (not (at_end l)) && is_ident_char (peek l) do
            Buffer.add_char buf (advance l)
          done;
          if (not (at_end l)) && peek l = '`' then ignore (advance l)
          else error l "expected closing backtick for infix operator";
          make_token l (BACKTICK_INFIX (Buffer.contents buf)) loc
        end
        else error l "expected identifier after backtick"
