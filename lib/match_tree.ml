...
let rec normalize_pattern type_env occ occ_ty (pat : Ast.pattern) :
    pattern_cell * binding list =
  match pat with
  ...
  | Ast.PatCons (hd, tl) ->
      let elem_ty =
        match Types.repr occ_ty with Types.TList t -> t | _ -> occ_ty
      in
      let hd_cell, hd_binds =
        normalize_pattern type_env (occ @ [ AConsHead ]) elem_ty hd
      in
      let tl_cell, tl_binds =
        normalize_pattern type_env (occ @ [ AConsTail ]) occ_ty tl
      in
  ...
  | Ast.PatTuple pats ->
      ...
      let cells_binds =
        List.mapi
          (fun i pat ->
            let ety =
              if i < List.length elem_tys then List.nth elem_tys i else occ_ty
            in
  ...
  | Ast.PatRecord fields ->
      ...
      let cells_binds =
        List.map
          (fun (name, pat) ->
            let idx =
              match List.assoc_opt name field_index_map with
              | Some i -> i
              | None -> 0
            in
            let fty = field_ty name in
            let cell, binds =
              normalize_pattern type_env
                (occ @ [ ARecordField (name, idx) ])
                fty pat
            in
            ((name, idx, cell), binds))
          fields
  ...
  | Ast.PatArray pats ->
      let elem_ty =
        match Types.repr occ_ty with Types.TArray t -> t | _ -> occ_ty
      in
      let cells_binds =
        List.mapi
          (fun i pat ->
      ...
      let rec build_map_cells = function
        ...
        | (key_pat, val_pat) :: rest ->
            let mk =
              ...
              | _ ->
                  failwith
                    "match_tree: map pattern key must be a literal or pin"
            in
            let val_ty = occ_ty in
            let val_occ = occ @ [ AMapValue mk ] in
            let val_cell, val_binds =
              normalize_pattern type_env val_occ val_ty val_pat
            in
            let _rest_cell, rest_binds = build_map_cells rest in
            (PCMapHas (mk, val_cell), val_binds @ rest_binds)
      in
      build_map_cells entries
  | Ast.PatPin name -> (PCPin name, [])
  | Ast.PatAnnot (inner, _) -> normalize_pattern type_env occ occ_ty inner
  | Ast.PatSet _ ->
      failwith "match_tree: PatSet should be desugared before lowering"

(* ---- Matrix operations ---- *)

let build_matrix type_env scrutinee_ty arms =
  (* Count max map/set key checks across all arms for column expansion *)
  let count_map_keys pat =
    match pat with Ast.PatMap entries -> max 1 (List.length entries) | _ -> 1
  in
  let max_cols =
    List.fold_left (fun acc (pat, _, _) -> max acc (count_map_keys pat)) 1 arms
  in
  List.mapi
    (fun arm_idx (pat, guard, _body) ->
      let cells, bindings =
        match pat with
        | Ast.PatMap entries when List.length entries > 1 ->
            (* Expand multi-key map into separate columns, each checking one key *)
            let cells_binds =
              List.map
                (fun entry ->
                  normalize_pattern type_env [] scrutinee_ty
                    (Ast.PatMap [ entry ]))
                entries
            in
            (List.map fst cells_binds, List.concat_map snd cells_binds)
        | _ ->
            let cell, bindings =
              normalize_pattern type_env [] scrutinee_ty pat
            in
            ([ cell ], bindings)
      in
      let padded =
        cells @ List.init (max_cols - List.length cells) (fun _ -> PCWild)
      in
      { pats = padded; arm_idx; guard; bindings })
    arms
  ...
  List.concat_map
    (fun row ->
      ...
      | Some (i, p1, binds1, p2, binds2) ->
          let replace pats p =
            List.mapi (fun j c -> if j = i then p else c) pats
          in
          let row1 =
            {
              row with
              pats = replace row.pats p1;
              bindings = row.bindings @ binds1;
            }
          in
          let row2 =
            {
              row with
              pats = replace row.pats p2;
              bindings = row.bindings @ binds2;
            }
          in
...
let specialize_row col head row =
  ...
  match h with
  | HCWild -> Some { row with pats = wild_subs @ remove_col row.pats }
  | _ when h = head ->
      let sub_pats =
        match cell with
...
let specialize col head rows = List.filter_map (specialize_row col head) rows
...
let default_matrix col rows =
  List.filter_map
    (fun row ->
      ...
      | HCWild ->
          Some { row with pats = List.filteri (fun i _ -> i <> col) row.pats }
      | _ -> None)
    rows
...
let distinct_heads col rows =
  let heads =
    List.filter_map
      (fun row ->
        let cell = List.nth row.pats col in
        match head_of cell with HCWild -> None | h -> Some h)
      rows
  in
    ...
    if num_cols <= 1 then 0
      ...
      let score col =
        ...
        let num_wilds =
          List.length
            (List.filter
               (fun row -> head_of (List.nth row.pats col) = HCWild)
               rows)
        in
...
type col_info = { col_occ : occurrence; col_ty : Types.ty }
  ...
  let new_cols =
    ...
    | HCConstructor (name, _, arity) ->
        if arity = 0 then []
          ...
          [
            {
              col_occ = parent.col_occ @ [ AVariantPayload ];
              col_ty = payload_ty;
            };
          ]
    | HCPolyVariant (tag_name, _, arity) ->
        if arity = 0 then []
          ...
          let payload_ty =
            match Types.repr parent.col_ty with
            | Types.TPolyVariant row ->
                let rec find = function
                  | Types.PVRow (n, ty_opt, _) when n = tag_name -> (
                      match ty_opt with Some t -> t | None -> Types.TUnit)
    ...
    | HCTuple n ->
        ...
        List.mapi
          (fun i _ ->
            let ty =
              if i < List.length elem_tys then List.nth elem_tys i
              else parent.col_ty
            in
    ...
    | HCMapHas mk ->
        let val_ty = parent.col_ty in
        (* best approximation; map value type *)
...
let expand_col_info_record col_infos col fields parent_ty _type_env =
  ...
  let remove_col lst = List.filteri (fun i _ -> i <> col) lst in
  let field_ty name =
    match Types.repr parent_ty with
    | Types.TRecord row ->
        let rec find = function
          | Types.RRow (n, ty, _) when n = name -> ty
          | Types.RRow (_, _, tail) -> find tail
...
let head_to_test = function
  ...
  | HCTuple _ | HCRecord | HCWild ->
      failwith "match_tree: no test for irrefutable/wild"
...
let rec compile type_env col_infos rows loc =
  ...
  | first_row :: _ ->
      let all_wild =
        List.for_all (fun p -> head_of p = HCWild) first_row.pats
      in
      if all_wild || first_row.pats = [] then
        ...
        if has_record then compile_record type_env col_infos rows col loc
        else
          let heads = distinct_heads col rows in
          let has_tuple =
            List.exists
              (fun h -> match h with HCTuple _ -> true | _ -> false)
              heads
          in
          ...
          if has_tuple && List.length heads = 1 then begin
            ...
            let complete =
              is_complete_signature type_env heads col_info.col_ty
            in
            ...
            let cases =
              List.map
                (fun head ->
                  let specialized = specialize col head rows in
                  let new_col_infos =
                    expand_col_info type_env col_infos col head
                  in
                  let sub_tree =
                    compile type_env new_col_infos specialized loc
                  in
            ...
            let default =
              if complete then None
                ...
                if def_rows = [] then Some (DFail loc)
                  ...
                  Some
                    (compile type_env
                       (List.filteri (fun i _ -> i <> col) col_infos)
                       def_rows loc)
            in
...
let compile_match type_env (scrutinee : Typechecker.texpr) arms match_kind loc =
  ...
  (* Determine number of columns from matrix (may be >1 for multi-key map patterns) *)
  let num_cols =
    match matrix with row :: _ -> List.length row.pats | [] -> 1
  in
  let col_infos =
    List.init num_cols (fun _ -> { col_occ = []; col_ty = scrutinee_ty })
  in
  let tree = compile type_env col_infos matrix loc in
  let match_arms =
    Array.of_list (List.map (fun (_, _, body) -> { arm_body = body }) arms)
  in
  { scrutinee; scrutinee_ty; match_arms; tree; loc; match_kind }
  ...
  | Typechecker.TEMatch (scrutinee, arms, match_kind) ->
      ...
      let arms' =
        List.map
          (fun (pat, guard, body) ->
            ( pat,
              Option.map (lower_texpr type_env) guard,
              lower_texpr type_env body ))
          arms
      in
...
let rec lower_program type_env (program : Typechecker.tprogram) :
    Typechecker.tprogram =
  List.map
    (fun decl ->
      match decl with
      | Typechecker.TDLet (n, e) -> Typechecker.TDLet (n, lower_texpr type_env e)
      | Typechecker.TDLetMut (n, e) ->
          Typechecker.TDLetMut (n, lower_texpr type_env e)
      | Typechecker.TDLetRec (n, e) ->
          Typechecker.TDLetRec (n, lower_texpr type_env e)
      | Typechecker.TDExpr e -> Typechecker.TDExpr (lower_texpr type_env e)
      | Typechecker.TDLetRecAnd binds ->
          Typechecker.TDLetRecAnd
            (List.map (fun (n, e) -> (n, lower_texpr type_env e)) binds)
