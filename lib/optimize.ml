...
type action = Keep | Remove | ReplaceWith of Bytecode.opcode list
...
let collect_jump_targets code =
  ...
  Array.iter
    (fun op ->
      ...
      | Bytecode.ENTER_LOOP t
      | Bytecode.LOOP_CONTINUE t ->
...
let is_jump_target targets offset = Hashtbl.mem targets offset
...
let mark_jump_to_next code actions =
  ...
  for i = 0 to len - 1 do
    match code.(i) with
    | Bytecode.JUMP target when target = i + 1 -> actions.(i) <- Remove
    | _ -> ()
  done
...
let reads_slot op n =
  match op with
  | Bytecode.GET_LOCAL s
  | Bytecode.GET_LOCAL_CALL (s, _)
  | Bytecode.GET_LOCAL_TUPLE_GET (s, _)
  | Bytecode.GET_LOCAL_FIELD (s, _) ->
      s = n
...
let writes_slot op n =
  match op with Bytecode.SET_LOCAL s -> s = n | _ -> false
  ...
  while !i < len && not !done_ do
    if actions.(!i) = Remove then incr i
      ...
      if reads_slot op n then (
        ...
        | Bytecode.TAIL_CALL_N _ | Bytecode.MATCH_FAIL _ | Bytecode.HALT ->
        ...
        | Bytecode.LOOP_BREAK | Bytecode.LOOP_CONTINUE _
        | Bytecode.FOLD_CONTINUE _ | Bytecode.ENTER_LOOP _
        | Bytecode.JUMP_TABLE _ ->
            result := false;
            done_ := true
        | _ -> incr i
      end
  ...
  while !i < len do
    ...
    if not !handled then begin
      ...
      | Bytecode.SET_LOCAL n
        when actions.(!i) = Keep && is_slot_dead code actions targets n (!i + 1)
        ->
  ...
  while !i < len do
    ...
    then begin
      ...
      | Bytecode.GET_LOCAL slot, Bytecode.TUPLE_GET idx ->
          actions.(!i) <-
            ReplaceWith [ Bytecode.GET_LOCAL_TUPLE_GET (slot, idx) ];
          actions.(!i + 1) <- Remove;
          i := !i + 2
...
let detect_tag_chain code _targets start_pos scrut_slot =
  ...
  while !continue do
    if
      ...
      &&
      match code.(!pos + 2) with
      | Bytecode.JUMP_IF_FALSE t -> t > !pos
      | _ -> false
    then begin
      let tag =
        match code.(!pos + 1) with Bytecode.TAG_EQ t -> t | _ -> assert false
      in
      let next =
        match code.(!pos + 2) with
  ...
  if valid && arms <> [] then begin
    ...
    if List.length unique_tags <> List.length tags then None
    else begin
              ...
              List.iter
                (fun (_, arm_body) ->
                  ...
                  if chain_start >= 0 && chain_start < len then begin
                    if actions.(chain_start) = Keep then
                      actions.(chain_start) <- Remove;
                    if chain_start + 1 < len && actions.(chain_start + 1) = Keep
                    then actions.(chain_start + 1) <- Remove;
                    if chain_start + 2 < len && actions.(chain_start + 2) = Keep
                    then actions.(chain_start + 2) <- Remove
                  end)
                remaining;
  ...
  while !i < len - 1 do
    ...
    then begin
      let is_pure_push =
        ...
        | Bytecode.GET_UPVALUE _ | Bytecode.NIL | Bytecode.DUP ->
            true
        | _ -> false
      in
      match code.(!i + 1) with
...
let thread_jumps code =
  ...
  if len = 0 then ()
    ...
    let resolve_target target =
      ...
      let continue = ref true in
      while !continue do
        if !t < 0 || !t >= len || Hashtbl.mem visited !t then continue := false
        else begin
          match code.(!t) with
          | Bytecode.JUMP next ->
    ...
    for i = 0 to len - 1 do
      ...
      | Bytecode.JUMP target ->
          ...
          if final_target >= 0 && final_target < len && control_depth.(i) = 0
          then begin
            match code.(final_target) with
            | Bytecode.RETURN -> code.(i) <- Bytecode.RETURN
            | _ ->
...
let rec optimize_proto (proto : Bytecode.prototype) : Bytecode.prototype =
  ...
  let new_code, new_lines =
    optimize_code new_constants proto.code proto.line_table
  in
