...
let serialize_opcode = function
  ...
  | CLOSURE_REC (i, caps, self) ->
      Printf.sprintf "[\"CLOSURE_REC\",%d,%s,%d]" i (serialize_captures caps)
        self
  ...
  | SET_FIELD name ->
      Printf.sprintf "[\"SET_FIELD\",%s]" (json_escape_string name)
  ...
  | MAKE_VARIANT (tag, name, has_payload) ->
      Printf.sprintf "[\"MAKE_VARIANT\",%d,%s,%b]" tag (json_escape_string name)
        has_payload
  | CONS -> "[\"CONS\"]"
  | NIL -> "[\"NIL\"]"
  ...
  | MATCH_FAIL loc ->
      Printf.sprintf "[\"MATCH_FAIL\",%s]" (json_escape_string loc)
...
let rec serialize_value = function
  ...
  | VVariant (tag, name, payload) ->
      let payload_str =
        match payload with None -> "null" | Some v -> serialize_value v
      in
      Printf.sprintf "{\"t\":\"V\",\"tag\":%d,\"name\":%s,\"payload\":%s}" tag
        (json_escape_string name) payload_str
  | _ -> failwith "serialize_value: unsupported value type in constants"

and serialize_prototype proto =
  let code_strs =
    Array.to_list (Array.map serialize_opcode proto.Bytecode.code)
  in
  let const_strs =
    Array.to_list (Array.map serialize_value proto.Bytecode.constants)
  in
  let line_strs =
    Array.to_list (Array.map string_of_int proto.Bytecode.line_table)
  in
  Printf.sprintf
    "{\"name\":%s,\"arity\":%d,\"num_locals\":%d,\"code\":[%s],\"constants\":[%s],\"line_table\":[%s]}"
    (json_escape_string proto.name)
    proto.arity proto.num_locals
...
let serialize_native_global idx value =
  ...
  | Bytecode.VRecord (shape, values) ->
      ...
      let all_external =
        Array.for_all
          (fun v -> match v with Bytecode.VExternal _ -> true | _ -> false)
          values
      in
...
let serialize_native_ext idx name arity =
  Printf.sprintf "%s:{\"type\":\"external\",\"name\":%s,\"arity\":%d}"
    (json_escape_string (string_of_int idx))
    (json_escape_string name) arity

let serialize_native_dict idx fields =
  let field_strs =
    List.map
      (fun (name, ext_name, arity) ->
        Printf.sprintf "%s:{\"name\":%s,\"arity\":%d}" (json_escape_string name)
...
let serialize_bundle ~global_names ~native_globals_json ~setup_protos
    ~main_proto =
