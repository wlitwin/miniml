...
let peek_kind_at p offset =
  ...
  if idx < Array.length p.tokens then p.tokens.(idx).kind else Token.EOF
  ...
  if tok.kind = kind then ignore (advance p)
    ...
    error p
      (Printf.sprintf "expected %s, got %s"
         (Token.describe_token_kind kind)
         (Token.describe_token_kind tok.kind))
...
let at_expr_start p =
  ...
  | Token.BYTE _ | Token.RUNE _ | Token.TRUE | Token.FALSE | Token.IDENT _
  | Token.UIDENT _ | Token.LPAREN | Token.LBRACE | Token.LBRACKET | Token.FN
  | Token.IF | Token.LET | Token.MATCH | Token.MINUS | Token.NOT | Token.LNOT
  | Token.PERFORM | Token.HANDLE | Token.TRY | Token.PROVIDE | Token.RESUME
  | Token.FOR | Token.BREAK | Token.CONTINUE | Token.RETURN | Token.HASH
  | Token.DO | Token.POLYTAG _ ->
      true
  | _ -> false
...
and parse_ty_atom_inner p =
  ...
  | Token.LBRACE ->
      ...
      Ast.TyRecord
        ( List.map
            (fun (f : Ast.record_field_def) -> (f.rfd_name, f.rfd_type))
            fields,
          is_open )
  | Token.LBRACKET ->
      ignore (advance p);
      ...
      while match peek_kind p with Token.POLYTAG _ -> true | _ -> false do
        let tag =
          match (advance p).kind with Token.POLYTAG s -> s | _ -> assert false
        in
        let payload =
          if peek_kind p = Token.OF then (
            ignore (advance p);
            Some (parse_ty p))
          else None
        in
...
and parse_ty_postfix p =
  ...
  let rec loop ty =
    ...
    | Token.IDENT name when peek_kind_at p 1 <> Token.COLON ->
        ignore (advance p);
        let args = match ty with Ast.TyTuple parts -> parts | _ -> [ ty ] in
        loop (Ast.TyApp (args, name))
    | Token.UIDENT _ when peek_kind_at p 1 = Token.DOT ->
        (* ('a, 'b) Module.typename *)
        let mod_parts = ref [] in
        let first_mod =
          match (advance p).kind with Token.UIDENT s -> s | _ -> assert false
        in
        mod_parts := [ first_mod ];
        ...
        if !final_name = "" then
          ...
          let qualified =
            String.concat "." (List.rev !mod_parts) ^ "." ^ !final_name
          in
          let args = match ty with Ast.TyTuple parts -> parts | _ -> [ ty ] in
  ...
  | Token.ARROW ->
      ...
      let eff =
        if peek_kind p = Token.SLASH then begin
          ...
          else Some (Ast.EffAnnotRow (parse_eff_items p))
        end
        else None
      in
...
and parse_pattern_atom_inner p =
  ...
  | Token.UIDENT s ->
      ...
      let arg =
        if at_pattern_arg_start p then Some (parse_pattern_atom p) else None
      in
  ...
  | Token.POLYTAG tag ->
      ...
      else Ast.PatPolyVariant (tag, None)
  | Token.CARET ->
      ignore (advance p);
      let name = expect_ident p in
      Ast.PatPin name
  | Token.HASH -> (
      ...
      | Token.LBRACE ->
          ...
          if peek_kind p = Token.RBRACE then begin
            ...
            (* Parse first key pattern, then decide: colon → map, else → set *)
            let first_pat = parse_pattern_atom p in
            if peek_kind p = Token.COLON then begin
              (* Map pattern: #{k: v; ...} *)
              ignore (advance p);
              let first_val = parse_pattern p in
              let rest = ref [] in
              ...
              expect p Token.RBRACE;
              Ast.PatMap ((first_pat, first_val) :: List.rev !rest)
            end
            else begin
              (* Set pattern: #{a; b; ...} *)
              let elems = ref [ first_pat ] in
              while peek_kind p = Token.SEMICOLON do
                ignore (advance p);
                if peek_kind p <> Token.RBRACE then
                  elems := parse_pattern_atom p :: !elems
              done;
              expect p Token.RBRACE;
              Ast.PatSet (List.rev !elems)
            end
          end
...
and at_pattern_arg_start p =
  ...
  | Token.INT _ | Token.TRUE | Token.FALSE | Token.STRING _ | Token.IDENT _
  | Token.UIDENT _ | Token.LPAREN | Token.LBRACKET | Token.LBRACE
  | Token.UNDERSCORE | Token.HASH | Token.POLYTAG _ | Token.CARET ->
      true
  | _ -> false
...
and parse_pattern_list_elems p =
  ...
  while peek_kind p = Token.SEMICOLON do
    ignore (advance p);
    if peek_kind p <> Token.RBRACKET then pats := parse_pattern p :: !pats
  done;
  List.rev !pats
...
let bp_of_binop = function
  ...
  | Token.EQ | Token.NEQ | Token.LT | Token.GT | Token.LE | Token.GE ->
      (6, 7) (* non-assoc, using left-assoc here *)
  ...
  | Token.BACKTICK_INFIX _ ->
      (8, 9) (* left-assoc, between comparison and cons *)
  | _ -> (-1, -1)

let is_binop = function
  | Token.PLUS | Token.MINUS | Token.STAR | Token.SLASH | Token.MOD | Token.EQ
  | Token.NEQ | Token.LT | Token.GT | Token.LE | Token.GE | Token.AMPAMP
  | Token.PIPEPIPE | Token.CARET | Token.COLONCOLON | Token.LAND | Token.LOR
  | Token.LXOR | Token.LSL | Token.LSR | Token.PIPEARROW
  | Token.BACKTICK_INFIX _ ->
      true
  | _ -> false
...
let resolve_fun_params ~is_generated (fun_params : fun_param list)
    (body : Ast.expr) : Ast.param list * Ast.expr =
  let body =
    List.fold_left
      (fun body fp ->
        match fp with
...
let parse_format_spec spec expr =
  ...
  while !i < len && spec.[!i] >= '0' && spec.[!i] <= '9' do
    incr i
  done;
  let width =
  ...
  let float_prec =
    if !i < len && spec.[!i] = '.' then begin
      ...
      let n =
        if !i > ps then int_of_string (String.sub spec ps (!i - ps)) else 0
      in
      if !i < len && spec.[!i] = 'f' then (
        incr i;
        Some n)
      else None
    end
  ...
  let inner =
    if zero_pad then
      match width with
      | Some w ->
          Ast.EApp (Ast.EApp (Ast.EVar "__fmt_zero_pad", Ast.EInt w), inner)
      | None -> inner
    else inner
  in
  ...
  let inner =
    if not zero_pad then
      ...
      | Some '>', Some w ->
          Ast.EApp (Ast.EApp (Ast.EVar "__fmt_pad_left", Ast.EInt w), inner)
      | Some '<', Some w ->
          Ast.EApp (Ast.EApp (Ast.EVar "__fmt_pad_right", Ast.EInt w), inner)
      | _ -> inner
    else inner
  in
  inner
    ...
    if peek_kind p = Token.SLASH then begin
      ...
      let eff =
        if peek_kind p = Token.IDENT "pure" then (
          ignore (advance p);
          Ast.EffAnnotPure)
        else Ast.EffAnnotRow (parse_eff_items p)
      in
...
and parse_atom_inner p =
  ...
  | Token.INTERP_STRING parts -> (
      ...
      | first :: rest ->
          List.fold_left
            (fun acc e -> Ast.EBinop (Ast.Concat, acc, e))
            first rest)
  | Token.TRUE ->
      ignore (advance p);
      Ast.EBool true
  ...
  | Token.POLYTAG tag ->
      ...
      else Ast.EPolyVariant (tag, None)
  | Token.DO ->
      ignore (advance p);
  ...
  | Token.BREAK ->
      ignore (advance p);
      if at_atom_start p then Ast.EBreak (Some (parse_atom p))
...
and parse_app p =
  ...
  let rec loop expr =
    ...
    | _ when at_atom_start p && not (is_keyword_expr_start p) -> (
        ...
        match expr with
        | Ast.EConstruct (name, None) | Ast.ELoc (_, Ast.EConstruct (name, None))
          ->
...
and is_keyword_expr_start p =
  match peek_kind p with
  | Token.FN | Token.IF | Token.LET | Token.MATCH | Token.PERFORM | Token.HANDLE
  | Token.TRY | Token.PROVIDE | Token.RESUME | Token.FOR | Token.BREAK
...
and parse_expr p =
  ...
  | Token.COLONEQUAL -> (
      ...
      | Token.SEMICOLON when peek_next_kind p <> Some Token.SEMICOLON ->
          ignore (advance p);
          if at_expr_start p then Ast.ESeq (assign, parse_expr p) else assign
      | _ -> assign)
  | Token.SEMICOLON when peek_next_kind p <> Some Token.SEMICOLON ->
      ignore (advance p);
...
and peek_next_kind p =
  if p.pos + 1 < Array.length p.tokens then Some p.tokens.(p.pos + 1).kind
  else None
...
and parse_fun_param_inner p =
  ...
  | Token.LPAREN ->
      ...
      if peek_kind p = Token.RPAREN then begin
        ...
        | Ast.PatVar name, _ ->
            FPParam Ast.{ name; annot; is_generated = false }
        | _, _ ->
...
and parse_fun p =
  ...
  if peek_kind p = Token.PIPE then begin
    ...
    let body = parse_expr p in
    let params, body =
      resolve_fun_params ~is_generated:false !fun_params body
    in
    List.fold_left (fun body param -> Ast.EFun (param, body)) body params
  end
...
and parse_let_expr p =
  ...
  if (not is_rec) && (not is_mut) && is_destruct_start p then begin
    ...
    if is_mut then begin
      ...
      let body = parse_expr p in
      let body =
        match annot with Some ty -> Ast.EAnnot (body, ty) | None -> body
      in
      expect p Token.IN;
      ...
      if is_rec && peek_kind p = Token.AND then begin
        ...
        while peek_kind p = Token.AND do
          ...
          let and_full_body =
            wrap_params_expr (List.rev !and_params) and_ret_annot and_body
          in
...
and parse_provide_expr p =
  ...
  while !continue_parsing do
    ...
    | Token.IDENT s ->
        ...
        arms :=
          Ast.HOp { op_name = s; arg = arg_name; k = "__k"; body = resume_expr }
          :: !arms;
        if peek_kind p = Token.PIPE then ignore (advance p)
        else continue_parsing := false
    | _ -> continue_parsing := false
...
and parse_set_or_map p =
  if peek_kind p = Token.RBRACE then
    ...
    if peek_kind p = Token.COLON then begin
    ...
    else if peek_kind p = Token.WITH then begin
      ...
      while peek_kind p = Token.SEMICOLON do
        ignore (advance p);
        if peek_kind p <> Token.RBRACE then elems := parse_expr_bp p 0 :: !elems
      done;
      `Set (List.rev !elems)
    end
  end
...
and parse_list_elems p =
  ...
  while peek_kind p = Token.SEMICOLON do
    ignore (advance p);
    if peek_kind p <> Token.RBRACKET then elems := parse_expr_bp p 0 :: !elems
  done;
  List.rev !elems

(* ---- Top-level declarations ---- *)

let is_tyvar_token = function Token.TYVAR _ -> true | _ -> false
...
let parse_deriving p =
  if peek_kind p = Token.DERIVING then begin
    ...
    let continue = ref true in
    while
      !continue && match peek_kind p with Token.UIDENT _ -> true | _ -> false
    do
...
let parse_op_or_ident p =
  if peek_kind p = Token.LPAREN then begin
    ignore (advance p);
    let name =
      match token_to_op_name (peek_kind p) with
      | Some n -> n
...
let parse_variant_ctors p =
  ...
  while !continue do
    ...
    if peek_kind p = Token.COLON then begin
      ...
      let arg =
        if peek_kind p = Token.OF then begin
          ignore (advance p);
          Some (parse_ty p)
        end
        else None
...
let parse_type_rhs p =
  ...
  | Token.LBRACE ->
      ...
      let fields, is_open = parse_ty_record_fields p in
      if is_open then
        error p "open record types (..) are not supported in type definitions";
      expect p Token.RBRACE;
      Ast.TDRecord fields
  | Token.PIPE | Token.UIDENT _ -> parse_variant_ctors p
  | _ -> Ast.TDAlias (parse_ty p)
...
let parse_type_decl p =
  ...
  if peek_kind p <> Token.AND then
    ...
    while peek_kind p = Token.AND do
      ...
      defs :=
        {
          Ast.td_params = and_type_params;
          td_name = and_name;
          td_def = and_def;
          td_deriving = and_deriving;
        }
        :: !defs
    done;
  ...
  Ast.DType
    {
      td_params = type_params;
      td_name = name;
      td_def = Ast.TDNewtype (ctor_name, underlying);
      td_deriving = deriving;
    }
...
let rec extract_pat_vars = function
  ...
  | Ast.PatRecord fields ->
      List.concat_map (fun (_, p) -> extract_pat_vars p) fields
  ...
  | Ast.PatMap entries ->
      List.concat_map
        (fun (k, v) -> extract_pat_vars k @ extract_pat_vars v)
        entries
  | Ast.PatSet elems -> List.concat_map extract_pat_vars elems
            ...
            Ast.DLet
              {
                ...
                constraints = [];
                body =
                  Ast.EMatch (Ast.EVar tmp, [ (pat, None, Ast.EVar v) ], Partial);
              })
          vars
    end
  end
  else if is_mut then begin
    let name = expect_ident p in
    let annot =
      if peek_kind p = Token.COLON then begin
        ignore (advance p);
    ...
    if peek_kind p = Token.IN then begin
      ...
      if is_rec && peek_kind p = Token.AND then begin
        ...
        while peek_kind p = Token.AND do
          ...
          let and_params, and_body =
            resolve_fun_params ~is_generated:true and_fun_params and_body
          in
          bindings :=
            {
              Ast.lr_name = and_name;
              type_params = and_type_params;
              params = and_params;
              ret_annot = and_ret_annot;
              constraints = and_constraints;
              body = and_body;
            }
            :: !bindings
        done;
          ...
          let expr_bindings =
            List.map
              (fun (b : Ast.letrec_binding) ->
                let full =
                  wrap_params_expr
                    (List.map (fun p -> FPParam p) b.params)
                    b.ret_annot b.body
                in
        ...
        else [ Ast.DLetRecAnd (List.rev !bindings) ]
      end
      else if is_rec then
        [
          Ast.DLetRec
      ...
      else [ Ast.DLet { name; params; ret_annot; constraints; body } ]
    end
  end
...
let parse_class_decl p =
  ...
  if peek_kind p = Token.DOT then
    error p
      "class declarations cannot use qualified names; define the class inside \
       a module instead";
...
let rec parse_module_body_item p : Ast.module_decl list =
  ...
  | Token.OPAQUE ->
      ...
      let decl =
        if peek_kind p = Token.NEWTYPE then parse_newtype_decl p
        else parse_type_decl p
      in
      [ Ast.{ vis = Opaque; decl } ]
  | _ ->
      let decls = parse_inner_decl p in
