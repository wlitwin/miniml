...
and texpr_kind =
  ...
  | TERecordUpdateIdx of
      texpr * (texpr * texpr) list (* base, [(idx_expr, val_expr)] *)
...
and thandle_arm =
  ...
  | THOpProvide of
      string * string * texpr (* tail-resumptive: op, arg, value_expr *)
  | THOpTry of
      string * string * texpr (* non-resuming: op, arg, fallback_expr *)
...
type tdecl =
  ...
  | TDModule of
      string
      * tdecl list
      * (string * Types.scheme) list (* name, decls, all member schemes *)
...
let rec strip_loc (expr : Ast.expr) =
  match expr with Ast.ELoc (_, inner) -> strip_loc inner | e -> e
...
let rec is_constructive (expr : Ast.expr) =
  ...
  | Ast.ERune _ | Ast.EUnit | Ast.ENil ->
      true
  | Ast.EVar _ -> true
  | Ast.EFun _ -> true
  ...
  | Ast.ERecord fields -> List.for_all (fun (_, e) -> is_constructive e) fields
  ...
  | Ast.EMap entries ->
      List.for_all
        (fun (k, v) -> is_constructive k && is_constructive v)
        entries
  | Ast.EMapTyped (_, entries) ->
      List.for_all
        (fun (k, v) -> is_constructive k && is_constructive v)
        entries
  | Ast.ECollTyped (_, entries) -> List.for_all is_constructive entries
  | Ast.ERecordUpdate (base, fields) ->
      is_constructive base
...
let try_unify ctx t1 t2 =
  try Types.unify t1 t2 with Types.Unify_error msg -> error ctx msg

let try_unify_eff ctx e1 e2 =
  try Types.unify_eff e1 e2 with Types.Unify_error msg -> error ctx msg
...
let rec max_tgen_in_ty = function
  | Types.TGen i -> i
  | Types.TArrow (a, _, b) | Types.TCont (a, _, b) ->
      max (max_tgen_in_ty a) (max_tgen_in_ty b)
  | Types.TTuple ts ->
      List.fold_left (fun acc t -> max acc (max_tgen_in_ty t)) (-1) ts
  | Types.TList t -> max_tgen_in_ty t
  | Types.TRecord row -> max_tgen_in_rrow row
  | Types.TVariant (_, args) ->
      List.fold_left (fun acc t -> max acc (max_tgen_in_ty t)) (-1) args
...
let rec max_effgen_in_eff = function
  ...
  | Types.EffRow (_, params, tail) ->
      let ty_max =
        List.fold_left (fun acc t -> max acc (max_tgen_in_ty t)) (-1) params
      in
          ...
          match
            List.find_opt
              (fun cls ->
                ...
                && List.exists
                     (fun (m, _) -> String.equal m method_name)
                     cls.class_methods)
              ctx.type_env.Types.classes
          with
          | Some class_def ->
              let method_ty =
                List.assoc method_name class_def.Types.class_methods
              in
...
let extend_var ctx name scheme =
  {
    ctx with
    vars = (name, scheme) :: ctx.vars;
    mutable_vars = List.filter (fun n -> n <> name) ctx.mutable_vars;
  }

let extend_var_mono ctx name ty = extend_var ctx name (Types.mono ty)
...
let freshen_arrow_effects level ty =
  let rec go = function
    | Types.TArrow (a, eff, b) ->
        let eff' =
          match eff with Types.EffEmpty -> Types.new_effvar level | e -> e
        in
...
let rec resolve_ty_annot_shared ctx level tvars eff_tvars (annot : Ast.ty_annot)
    : Types.ty =
  let rec go = function
    | Ast.TyVar name -> (
    ...
    | Ast.TyName name -> (
        ...
        match
          List.find_opt
            (fun (n, _, _) -> String.equal n canonical)
            ctx.type_env.type_synonyms
        with
        | Some (_, 0, ty) ->
            freshen_tvars level (freshen_arrow_effects level ty)
        | Some (_, n, _) ->
            error ctx
              (Printf.sprintf "type %s expects %d type argument(s)" name n)
        | None -> (
            ...
            | None ->
                ...
                else error ctx (Printf.sprintf "unknown type: %s" name)))
    ...
    | Ast.TyApp (args, name) -> (
        ...
        | Some (_, np, ty) ->
            ...
            freshen_tvars level
              (freshen_arrow_effects level (subst_tgens arg_tys ty))
        | None -> (
            match find_variant_info ctx.type_env name with
            | Some (_, np, _, _) ->
                if List.length arg_tys <> np then
                  error ctx
                    (Printf.sprintf
                       "type %s expects %d type argument(s), got %d" name np
                       (List.length arg_tys));
    ...
    | Ast.TyRecord (fields, is_open) ->
        ...
        Types.TRecord
          (List.fold_right
             (fun (n, t) acc -> Types.RRow (n, t, acc))
             fields tail)
...
and resolve_eff_items ctx level tvars eff_tvars (items : Ast.eff_item list) :
    Types.eff =
  let labels = ref [] in
    ...
    (fun item ->
      ...
      | Ast.EffLabel (name, param_annots) ->
          ...
          if expected_params <> actual_params then
            error ctx
              (Printf.sprintf "effect %s expects %d type parameter(s), got %d"
  ...
  List.fold_right
    (fun (name, param_annots) acc ->
      let resolved_params =
        List.map
          (resolve_ty_annot_shared ctx level tvars eff_tvars)
          param_annots
      in
...
let resolve_ty_annot_def ~loc ~check_synonyms type_env resolve_var resolve_eff
    annot =
  let rec go = function
    ...
    | Ast.TyName name -> (
        ...
        | None -> (
            ...
            | None ->
                ...
                else error_at loc (Printf.sprintf "unknown type: %s" name)))
    | Ast.TyArrow (a, b, eff_opt) ->
    ...
    | Ast.TyApp (args, name) -> (
        ...
        | None -> (
            ...
            | None ->
                error_at loc
                  (Printf.sprintf "unknown type constructor: %s" name)))
    | Ast.TyRecord (fields, is_open) ->
        let fields = List.map (fun (n, t) -> (n, go t)) fields in
        let fields =
          List.sort (fun (a, _) (b, _) -> String.compare a b) fields
...
let instantiate_record_fields level fields =
  let max_gen =
    List.fold_left (fun acc (_, ty) -> max acc (max_tgen_in_ty ty)) (-1) fields
  in
  if max_gen < 0 then fields
  else
    let fresh = List.init (max_gen + 1) (fun _ -> Types.new_tvar level) in
    List.map
      (fun (n, ty) -> (n, freshen_arrow_effects level (subst_tgens fresh ty)))
      fields
...
let iter_texpr_children f te =
  ...
  | TERecordUpdate (base, overrides) ->
      f base;
      List.iter (fun (_, e) -> f e) overrides
  ...
  | TELetRecAnd (bindings, body) ->
      List.iter (fun (_, e) -> f e) bindings;
      f body
            ...
            Match_tree_types.DSwitch
              {
                ...
                cases =
                  List.map
                    (fun (test, binds, sub) -> (test, binds, map_tree sub))
                    sw.cases;
          ...
          List.map
            (fun arm ->
              ...
              | THOp { op_name; arg; k; body } ->
                  THOp { op_name; arg; k; body = f body }
...
let infer_implicit_constraints binding_name type_env vars te scheme =
  if scheme.Types.constraints <> [] || scheme.Types.quant = 0 then scheme
    ...
    let rec build_map s a =
      ...
      | Types.TTuple ss -> (
          match a with
          | Types.TTuple aa when List.length ss = List.length aa ->
              List.iter2 build_map ss aa
          | _ -> ())
      | Types.TList s1 -> (
          match a with Types.TList a1 -> build_map s1 a1 | _ -> ())
      | Types.TArray s1 -> (
          match a with Types.TArray a1 -> build_map s1 a1 | _ -> ())
      | Types.TVariant (_, ss) -> (
          match a with
          | Types.TVariant (_, aa) when List.length ss = List.length aa ->
              List.iter2 build_map ss aa
          | _ -> ())
    ...
    let set_add tbl key =
      if not (List.exists (fun k -> k = key) !tbl) then tbl := key :: !tbl
    in
    ...
    let find_class_method_for_var name =
      ...
      | Some class_def when not (String.equal name binding_name) -> (
          ...
          | Some s ->
              if s.Types.body = method_ty then (Some class_def, name)
              else (None, name)
          | None -> (None, name))
      | _ -> (
          ...
          | Some i -> (
              ...
              | Some class_def
                when (not (String.equal short binding_name))
                     && (String.equal class_def.Types.class_name class_prefix
                        || String.length class_def.Types.class_name
                           > String.length mod_prefix
                           && String.sub class_def.Types.class_name 0
                                (String.length mod_prefix)
                              = mod_prefix) -> (
                  let method_ty =
                    List.assoc short class_def.Types.class_methods
                  in
                  let var_sch =
                    match List.assoc_opt name vars with
                    | Some _ as r -> r
                  ...
                  match var_sch with
                  | Some s ->
                      if s.Types.body = method_ty then (Some class_def, short)
                      else (None, name)
                  | None -> (None, name))
              | _ -> (None, name)))
    in
    ...
    let record_class_constraint class_def method_name te_ty =
      ...
      let rec go s r =
        ...
        | Types.TCont (s1, _, s2), Types.TCont (r1, _, r2) ->
            go s1 r1;
            go s2 r2
      ...
      let cc_args =
        List.init num_params (fun i ->
            ...
            | Some (Types.TVar { contents = Types.Unbound (id, _) } as tv) -> (
                ...
                | _ ->
                    let is_fundep_target =
                      List.exists
                        (fun (fd : Types.fundep) -> List.mem i fd.fd_to)
            ...
            | None ->
                let is_fundep_target =
                  List.exists
                    ...
                    class_def.Types.class_fundeps
                in
                if is_fundep_target then Some Types.CAWild else None)
      in
      ...
      let has_any_caty =
        List.exists
          (fun opt -> match opt with Some (Types.CATy _) -> true | _ -> false)
          cc_args
      in
      let cc_args =
        if has_nonconc_caty then
          List.map
            (fun opt ->
              match opt with
      ...
      let allow =
        has_tgen && all_resolved
        && ((not has_caty)
           ||
           match scheme.Types.body with
      ...
      else if (not has_tgen) && all_resolved then begin
        let has_unbound_caty =
          List.exists
            (fun opt ->
              match opt with
              | Some (Types.CATy (Types.TVar { contents = Types.Unbound _ })) ->
                  true
              | _ -> false)
            cc_args
        in
      ...
      while !changed do
                  ...
                  (fun (ca : Types.class_arg) ->
                    ...
                    | Types.CATy
                        ...
                        List.exists
                          (fun (_c, fargs) ->
                            List.exists
                              (fun (fa : Types.class_arg) ->
                                match fa with
                                | Types.CATy
                                    (Types.TVar
                                       { contents = Types.Unbound (fid, _) }) ->
                                    fid = id
                                | Types.CAPhantom
                                    ( _,
                                      Types.TVar
        ...
        List.iter
          (fun (cls, args) ->
            ...
            in
            pending_caty :=
              List.filter (fun k -> k <> (cls, args)) !pending_caty;
    ...
    let propagate_constrained_ref local_sch actual_ty =
      ...
      let rec map_tgens s a =
        ...
        | Types.TTuple ss -> (
            match a with
            | Types.TTuple aa when List.length ss = List.length aa ->
                List.iter2 map_tgens ss aa
            | _ -> ())
        | Types.TList s1 -> (
            match a with Types.TList a1 -> map_tgens s1 a1 | _ -> ())
        | Types.TArray s1 -> (
            match a with Types.TArray a1 -> map_tgens s1 a1 | _ -> ())
        | Types.TVariant (_, ss) -> (
            match a with
            | Types.TVariant (_, aa) when List.length ss = List.length aa ->
                List.iter2 map_tgens ss aa
              ...
              (fun (ca : Types.class_arg) ->
                ...
                | CATGen tgen_idx -> (
                    ...
                    | Some
                      ...
                      -> (
                        match Hashtbl.find_opt id_map id with
                        | Some (Types.TGen outer_tgen) ->
                            Some (Types.CATGen outer_tgen)
                        | _ -> Some (Types.CATy tv))
                    | Some _ ->
                        None
                        (* Concrete type: resolve at inner level, don't propagate *)
                    | None -> None)
                | CATy ty -> (
               ...
               but constraints sharing the same inner phantom index stay linked. *)
                    let outer_idx, outer_tv =
                      match Hashtbl.find_opt phantom_remap inner_idx with
                      | Some pair -> pair
                      | None ->
                          let idx = !phantom_counter in
                          incr phantom_counter;
    ...
    let rec walk locals te =
      ...
      | TEVar name -> (
          ...
          | Some class_def ->
              record_class_constraint class_def method_name te.ty
          | None -> ())
      | _ -> ());
        ...
        | Types.TVar ({ contents = Types.Unbound (id, _) } as tv) -> (
            ...
            | _ -> (
                ...
                | Some phantom_idx ->
                    set_add found_multi
                      ( class_name,
                        [ Types.CAPhantom (phantom_idx, Types.TVar tv) ] )
                | None -> ()))
        | _ -> ()
      in
      match te.expr with
    ...
    and walk_children locals te =
      ...
      | TEUnit | TEVar _ | TENil | TEContinueLoop ->
          ()
      ...
      | TERecordUpdate (base, overrides) ->
          walk locals base;
          List.iter (fun (_, e) -> walk locals e) overrides
      | TERecordUpdateIdx (base, pairs) ->
          walk locals base;
          List.iter
            (fun (i, v) ->
      ...
      | TEHandle (body, arms) ->
          ...
          List.iter
            ...
            arms
      | TELetRecAnd (bindings, body) ->
          List.iter (fun (_, e) -> walk locals e) bindings;
          walk locals body
  ...
  if scheme.Types.rquant = 0 then scheme
    ...
    let rec walk_rmap s a =
      ...
      | Types.TTuple ss -> (
          match a with
          | Types.TTuple aa when List.length ss = List.length aa ->
              List.iter2 walk_rmap ss aa
          | _ -> ())
      | Types.TList s1 -> (
          match a with Types.TList a1 -> walk_rmap s1 a1 | _ -> ())
      | Types.TArray s1 -> (
          match a with Types.TArray a1 -> walk_rmap s1 a1 | _ -> ())
      | Types.TVariant (_, ss) -> (
          match a with
          | Types.TVariant (_, aa) when List.length ss = List.length aa ->
              List.iter2 walk_rmap ss aa
    ...
    let add_evidence rgen_idx fields =
      let existing =
        match Hashtbl.find_opt found_evidences rgen_idx with
        | Some fs -> fs
        | None -> []
      in
      let merged =
        List.fold_left
          (fun fs f -> if List.mem f fs then fs else f :: fs)
          existing fields
      in
      ...
      | TERecordUpdate (_base, overrides) -> (
          ...
          | Types.TRecord row -> (
              ...
              | Types.RVar { contents = Types.RUnbound (id, _) } -> (
                  ...
                  | Some rgen_idx ->
                      add_evidence rgen_idx (List.map fst overrides)
                  | None -> ())
              | _ -> ())
          | _ -> ())
                  ...
                  | Some arow -> (
                      ...
                      if is_open arow then
                        ...
                        | Types.RVar { contents = Types.RUnbound (id, _) } -> (
                            match Hashtbl.find_opt rvar_to_rgen id with
                            | Some caller_rgen ->
                                add_evidence caller_rgen re.re_fields
                            | None -> ())
                        | _ -> ())
                  | None -> ())
    ...
    let ev_list =
      Hashtbl.fold
        (fun rgen_idx fields acc ->
          {
            Types.re_fields = List.sort String.compare fields;
            re_rgen = rgen_idx;
          }
          :: acc)
        found_evidences []
...
let improve_fundeps_in_expr vars type_env te =
  ...
  let improve_constrained_call_with_scheme sch ty =
    ...
    let rec map_tgens s a =
      ...
      match s with
      | Types.TGen i ->
          if not (Hashtbl.mem tgen_to_actual i) then
            Hashtbl.replace tgen_to_actual i a
      | Types.TArrow (s1, _, s2) -> (
          match a with
          | Types.TArrow (a1, _, a2) ->
              map_tgens s1 a1;
    ...
    List.iter
      (fun (cc : Types.class_constraint) ->
        match
          List.find_opt
            (fun cls -> String.equal cls.Types.class_name cc.cc_class)
            classes
        with
        | None -> ()
        ...
        | Some class_def ->
            ...
            in
            let improved =
              Types.improve_with_fundeps instances class_def partial
            in
              ...
              (fun i opt ->
                if i < num_params then
                  ...
                  | Some resolved_ty, None -> (
                      ...
                      let tvar_opt =
                        match ca with
                        | Types.CATGen tgen_idx ->
                            Hashtbl.find_opt tgen_to_actual tgen_idx
                      ...
                      | Some (Types.TVar ({ contents = Types.Unbound _ } as r))
                        -> (
                          try Types.unify (Types.TVar r) resolved_ty
                          with Types.Unify_error _ -> ())
                      | _ -> ())
                  | _ -> ())
              improved)
    ...
    let class_opt, method_name =
      ...
      | None -> (
          ...
          | Some i -> (
              ...
              | Some class_def
                when String.equal class_def.Types.class_name class_prefix ->
                  (Some class_def, short)
              | Some class_def
                when String.length class_def.Types.class_name
                     > String.length mod_prefix
                     && String.sub class_def.Types.class_name 0
                          (String.length mod_prefix)
                        = mod_prefix ->
    ...
    | Some class_def when class_def.Types.class_fundeps <> [] ->
        ...
        let rec go s r =
          ...
          | Types.TCont (s1, _, s2), Types.TCont (r1, _, r2) ->
              go s1 r1;
              go s2 r2
          | Types.TTuple ss, Types.TTuple rs
            when List.length ss = List.length rs ->
              List.iter2 go ss rs
          | Types.TList s1, Types.TList r1 -> go s1 r1
          | Types.TArray s1, Types.TArray r1 -> go s1 r1
...
let rec is_syntactic_value (expr : Ast.expr) : bool =
  ...
  | Ast.ERecord fields ->
      List.for_all (fun (_, e) -> is_syntactic_value e) fields
  ...
  | Ast.EAnnot (e, _) | Ast.ECoerce (e, _) | Ast.ELoc (_, e) ->
...
let rec specialize_variant pats ctor_name =
  List.concat_map
    (fun pat ->
      match pat with
      | Ast.PatConstruct (name, sub)
        when String.equal (ctor_short_name name) ctor_name -> (
          match sub with
...
let has_wildcard pats =
  List.exists
    (fun p -> match p with Ast.PatWild | Ast.PatVar _ -> true | _ -> false)
    pats
  ...
  if has_wildcard pats then []
    ...
    | Types.TVariant (name, args) -> (
        ...
        | Some (_, _, ctors, is_gadt) ->
            ...
            let reachable_ctors =
              if is_gadt then
                List.filter
                  (fun (ctor_name, _) ->
                    match
                      List.assoc_opt ctor_name type_env.Types.constructors
                    with
                    | Some info -> (
                        match info.Types.ctor_return_ty_params with
                        | None ->
                            true (* regular ctor in GADT, always reachable *)
                        | Some ret_params ->
            ...
            in
            List.concat_map
              (fun (ctor_name, ctor_arg_ty) ->
                let sub_pats = specialize_variant pats ctor_name in
                match ctor_arg_ty with
                | None -> if List.length sub_pats = 0 then [ ctor_name ] else []
                | Some arg_ty ->
                    if List.length sub_pats = 0 then [ ctor_name ^ " _" ]
                    else
                      let sub_missing =
                        uncovered_patterns type_env arg_ty sub_pats
                      in
    ...
    | Types.TBool ->
        let has_true =
          List.exists
            (fun p -> match p with Ast.PatBool true -> true | _ -> false)
            pats
        in
        let has_false =
          List.exists
            (fun p -> match p with Ast.PatBool false -> true | _ -> false)
            pats
        in
        let missing = ref [] in
    ...
    | Types.TList _ ->
        let has_nil =
          List.exists
            (fun p -> match p with Ast.PatNil -> true | _ -> false)
            pats
        in
        let has_cons =
          List.exists
            (fun p -> match p with Ast.PatCons _ -> true | _ -> false)
            pats
        in
        let missing = ref [] in
    ...
    | Types.TArray _ ->
        ...
        let missing_pat =
          if missing_len = 0 then "#[]"
            ...
            "#["
            ^ String.concat "; " (List.init missing_len (fun _ -> "_"))
            ^ "]"
        in
        [ missing_pat ]
    ...
    | Types.TUnit ->
        let has_unit =
          List.exists
            (fun p -> match p with Ast.PatUnit -> true | _ -> false)
            pats
        in
    ...
    | Types.TTuple ts ->
        ...
        if tuple_pats = [] then
          ...
          let rec check_positions i = function
            ...
            | elem_ty :: rest_tys ->
                let position_pats =
                  List.map (fun ps -> List.nth ps i) tuple_pats
                in
                let sub_missing =
                  uncovered_patterns type_env elem_ty position_pats
                in
                if sub_missing <> [] then
                  let template =
                ...
                else check_positions (i + 1) rest_tys
          in
...
let exhaustiveness_check ctx ty arms =
  ...
  let unguarded_pats =
    List.filter_map
      (fun (pat, guard, _body) ->
        match guard with None -> Some pat | Some _ -> None)
      arms
  in
...
let rec synth ctx level (expr : Ast.expr) : texpr =
  ...
  | Ast.ENil -> mk ctx TENil (Types.TList (Types.new_tvar level))
  | Ast.EVar name ->
  ...
  | Ast.EFun (param, body) ->
      ...
      let has_return =
        if param.is_generated then false else !(ctx'.return_used)
      in
      mk ctx
        (TEFun (param.name, body_te, has_return))
        (Types.TArrow (param_ty, body_eff, ret_ty))
  | Ast.ELet (name, e1, e2) ->
      let e1_te = synth ctx (level + 1) e1 in
      improve_fundeps_in_expr ctx.vars ctx.type_env e1_te;
      let scheme =
        if is_syntactic_value e1 then Types.generalize level e1_te.ty
        else Types.mono e1_te.ty
      in
      let scheme =
        infer_implicit_constraints "" ctx.type_env ctx.vars e1_te scheme
      in
      let scheme = infer_record_evidence ctx.vars e1_te scheme in
      let stored_scheme =
        if scheme_needs_xform scheme then Some scheme else None
      in
  ...
  | Ast.ERecord fields ->
      ...
      let field_tys =
        List.sort (fun (a, _) (b, _) -> String.compare a b) field_tys
      in
  ...
  | Ast.EIndex (base, idx) -> (
      ...
      | _ ->
          ...
          | Some class_def when class_def.Types.class_fundeps <> [] -> (
              ...
              let improved =
                Types.improve_with_fundeps ctx.type_env.Types.instances
                  class_def partial
              in
  ...
  | Ast.EList (first :: rest) ->
      ...
      mk ctx
        (TECons
           ( first_te,
             List.fold_right
               (fun te acc -> mk ctx (TECons (te, acc)) list_ty)
               rest_tes (mk ctx TENil list_ty) ))
        list_ty
  | Ast.EList [] -> mk ctx TENil (Types.TList (Types.new_tvar level))
  | Ast.EArray (first :: rest) ->
      let first_te = synth ctx level first in
      let rest_tes = List.map (fun e -> check ctx level e first_te.ty) rest in
      mk ctx (TEArray (first_te :: rest_tes)) (Types.TArray first_te.ty)
  | Ast.EArray [] -> mk ctx (TEArray []) (Types.TArray (Types.new_tvar level))
        ...
        Ast.EApp
          ( Ast.EApp
              ( Ast.EApp
                  ( Ast.EVar "fold",
                    Ast.EFun
                      ( { name = acc_name; annot = None; is_generated = true },
                        Ast.EFun
                          ( {
                              name = var_name;
                              annot = None;
  ...
  | Ast.EWhileLet (pat, scrutinee, body) ->
      ...
      let desugared =
        Ast.EWhile
          {
            while_cond = Ast.EBool true;
            while_body =
              Ast.EMatch
                ( scrutinee,
                  [ (pat, None, body); (Ast.PatWild, None, Ast.EBreak None) ],
                  Total );
          }
      in
  ...
  | Ast.EMap pairs ->
      let pair_list =
        Ast.EList (List.map (fun (k, v) -> Ast.ETuple [ k; v ]) pairs)
      in
  ...
  | Ast.ELocalOpen (mod_name, inner_expr) ->
      ...
      let desugared =
        List.fold_left
          (fun body (short, _scheme) ->
            let qualified = mod_name ^ "." ^ short in
            Ast.ELet (short, Ast.EVar qualified, body))
          inner_expr minfo.Types.mod_pub_vars
      in
      synth ctx level desugared
  | Ast.ELetRecAnd (bindings, body) -> synth_let_rec_and ctx level bindings body
...
and synth_field ctx level e field =
  ...
  | Types.TVar _ -> (
      let candidates =
        List.filter
          (fun (_name, fields) -> List.mem_assoc field fields)
          ctx.type_env.Types.records
      in
      ...
      | _ ->
          try_unify ctx te.ty
            (Types.TRecord (Types.RRow (field, field_ty, row_tail))))
  | _ ->
      try_unify ctx te.ty
        (Types.TRecord (Types.RRow (field, field_ty, row_tail))));
...
and synth_let_rec ctx level name type_params e1 e2 =
  ...
  | Ast.EFun _ | Ast.EAnnot (Ast.EFun _, _) ->
      if type_params <> [] then begin
        ...
        let scheme =
          infer_implicit_constraints name ctx.type_env ctx.vars e1_te scheme
        in
        let scheme = infer_record_evidence ctx.vars e1_te scheme in
        let stored_scheme =
          if scheme_needs_xform scheme then Some scheme else None
  ...
  | _ when is_constructive e1 && not (is_immediately_linked name e1) ->
      if type_params <> [] then
        error ctx
          "recursive value binding does not support polymorphic type parameters";
      ...
      mk ctx (TELetRec (name, None, e1_te, e2_te)) e2_te.ty
  | _ ->
      error ctx "let rec binding must be a function or constructive expression"
...
and synth_record_update ctx level base overrides =
  ...
  | Types.TVar _ -> (
      ...
      | _ ->
          ...
          let row =
            List.fold_right
              (fun name acc -> Types.RRow (name, Types.new_tvar level, acc))
              override_names row_tail
          in
          try_unify ctx base_te.ty (Types.TRecord row))
  | Types.TRecord _ -> ()
  | _ -> error ctx "record update on non-record type");
  (* For each override, unify base with { field: 'a | ρ } to ensure field exists *)
  let override_field_tys =
    List.map
      (fun (name, _) ->
        let field_ty = Types.new_tvar level in
        let row_tail = Types.new_rvar level in
        try_unify ctx base_te.ty
          (Types.TRecord (Types.RRow (name, field_ty, row_tail)));
        let actual_ty =
          match Types.repr base_te.ty with
          | Types.TRecord row -> (
              let fields = Types.record_row_to_fields row in
  ...
  let typed_overrides =
    List.map2
      (fun (name, e) (_, expected_ty) ->
        let te = check ctx level e expected_ty in
        (name, te))
      overrides override_field_tys
  in
  ...
  let is_open =
    ...
    | _ -> false
  in
  if is_open then mk ctx (TERecordUpdate (base_te, typed_overrides)) base_te.ty
  else begin
    let field_tys =
      match Types.repr base_te.ty with
...
and synth_let_rec_and ctx level bindings body =
  ...
  if has_poly then begin
    ...
    let tvars_list =
      List.map
        (fun (_, type_params, params, ret_annot, _) ->
          ...
          let param_tys =
            List.map
              (fun (p : Ast.param) ->
                match p.annot with
                | Some annot ->
                    resolve_ty_annot_shared ctx (level + 1) shared_tvars
                      shared_eff_tvars annot
            ...
            | Some (Ast.TyWithEffect (ty, eff_annot)) ->
                ...
                let eff =
                  ...
                  | Ast.EffAnnotRow items ->
                      resolve_eff_items ctx (level + 1) shared_tvars
                        shared_eff_tvars items
                in
                (ty', eff)
            | Some annot ->
                ( resolve_ty_annot_shared ctx (level + 1) shared_tvars
    ...
    in
    let schemes =
      List.map (fun (_, _, _, fn_ty) -> Types.generalize level fn_ty) tvars_list
    in
    let ctx' =
      List.fold_left2
        (fun ctx (name, _, _, _, _) scheme -> extend_var ctx name scheme)
        ctx infos schemes
    in
    let body_tes =
      List.map2
        (fun (_, _, params, _, inner_body) (param_tys, ret_ty, body_eff, _) ->
          let inner_ctx =
            List.fold_left2
              (fun c (p : Ast.param) ty -> extend_var_mono c p.name ty)
              { ctx' with current_eff = body_eff }
              params param_tys
          in
          ...
          | (last_p : Ast.param) :: rest_ps, last_pty :: rest_ptys ->
              let inner =
                mk ctx
                  (TEFun (last_p.name, body_te, false))
                  (Types.TArrow (last_pty, body_eff, body_te.ty))
              in
              List.fold_left2
                (fun acc (p : Ast.param) pty ->
                  mk ctx
                    (TEFun (p.name, acc, false))
    ...
    let body_te = synth ctx'' level body in
    let typed_bindings =
      List.map2 (fun (name, _, _, _, _) te -> (name, te)) infos body_tes
    in
    ...
    List.iter
      (fun (name, _, fn_expr) ->
        ...
        | Ast.EFun _ | Ast.EAnnot (Ast.EFun _, _) -> ()
        | _
          when is_constructive fn_expr
               && not (is_immediately_linked name fn_expr) ->
            ()
        | _ ->
            error ctx
              (Printf.sprintf
                 "let rec binding for '%s' must be a function or constructive \
                  expression"
                 name))
      bindings;
    let fn_vars =
      List.map (fun (name, _, _) -> (name, Types.new_tvar (level + 1))) bindings
    in
    let ctx' =
      List.fold_left
        (fun ctx (name, tv) -> extend_var_mono ctx name tv)
        ctx fn_vars
    in
    let body_tes =
      List.map2
        (fun (_, _, fn_expr) (_, tv) ->
    ...
    let ctx'' =
      List.fold_left2
        (fun ctx (name, _, _) (_, tv) ->
          let scheme = Types.generalize level tv in
          extend_var ctx name scheme)
        ctx bindings fn_vars
    in
    let body_te = synth ctx'' level body in
    let typed_bindings =
      List.map2 (fun (name, _, _) te -> (name, te)) bindings body_tes
    in
    mk ctx (TELetRecAnd (typed_bindings, body_te)) body_te.ty
  end
...
and synth_perform ctx level op_name arg =
  ...
  | Some (effect_name, op_ty) -> (
      ...
      let edef =
        List.find
          (fun e -> String.equal e.Types.effect_name effect_name)
          ctx.type_env.Types.effects
      in
      (* Create fresh type vars for each effect type parameter *)
      let fresh_params =
        List.map (fun _ -> Types.new_tvar level) edef.Types.effect_params
      in
      ...
      let concrete_op_ty =
        if fresh_params = [] then concrete_op_ty
        else subst_tgens fresh_params concrete_op_ty
      in
      ...
      | Types.TArrow (param_ty, _, ret_ty) ->
          ...
          try_unify_eff ctx ctx.current_eff
            (Types.EffRow (effect_name, fresh_params, fresh_tail));
          mk ctx (TEPerform (op_name, arg_te)) ret_ty
      | _ ->
          error ctx
            (Printf.sprintf "effect operation %s must have function type"
               op_name))
...
and synth_handle ctx level body arms =
    ...
    List.filter_map
      (fun arm ->
        ...
        | Ast.HOp { op_name; _ } -> (
            ...
            | None ->
                error ctx
                  (Printf.sprintf "unknown effect operation in handler: %s"
                     op_name))
        | _ -> None)
      arms
  in
  ...
  let remaining_eff =
    List.fold_left
      (fun eff (effect_name, fresh_params) ->
        try Types.rewrite_row effect_name fresh_params eff
        with Types.Unify_error msg -> error ctx msg)
      (Types.eff_repr body_eff) effect_fresh_params
  in
        ...
        | Ast.HOp { op_name; arg = arg_name; k = k_name; body = handler_body }
          -> (
            ...
            | Some (effect_name, op_ty) ->
                ...
                in
                let concrete_op_ty =
                  if fresh_params = [] then op_ty
                  else subst_tgens fresh_params op_ty
                in
                let param_ty, ret_ty_op =
                  ...
                  | _ ->
                      error ctx
                        (Printf.sprintf
                           "effect operation %s must have function type" op_name)
                in
                let k_ty =
                  Types.TCont (ret_ty_op, ctx.current_eff, result_ty)
                in
...
and qualify_ctor_name type_env name info =
  ...
  if String.contains name '.' then name
    ...
    match String.rindex_opt info.Types.ctor_type_name '.' with
    | Some i ->
        let qualified = String.sub info.Types.ctor_type_name 0 (i + 1) ^ name in
        if List.assoc_opt qualified type_env.Types.constructors <> None then
          qualified
        else name
    | None -> name
...
and qualify_pattern type_env pat =
  let rec go pat =
    ...
    | Ast.PatRecord fields ->
        Ast.PatRecord (List.map (fun (f, p) -> (f, go p)) fields)
    | Ast.PatArray ps -> Ast.PatArray (List.map go ps)
    | Ast.PatMap kvs -> Ast.PatMap (List.map (fun (k, v) -> (go k, go v)) kvs)
    | Ast.PatSet elems ->
        (* Desugar set pattern to map pattern: #{a; b} → #{a: _; b: _}
         Sets are newtypes over maps, erased at runtime, so map operations work. *)
        Ast.PatMap (List.map (fun e -> (go e, Ast.PatWild)) elems)
...
and synth_construct ctx level name arg =
  ...
  | Some info -> (
      ...
      let fresh_universals =
        List.filteri (fun i _ -> i < info.ctor_num_params) all_fresh
      in
      let result_ty =
      ...
      | Some info ->
          if
            List.mem info.Types.ctor_type_name
              ctx.type_env.Types.hidden_ctor_types
          then
            error ctx (Printf.sprintf "unknown constructor in pattern: %s" name);
          let num_fresh = info.ctor_num_params + info.ctor_existentials in
          let all_fresh = List.init num_fresh (fun _ -> Types.new_tvar level) in
          (* Collect existential tvar refs (indices >= ctor_num_params) *)
          let existential_refs =
            List.filter_map
              (fun i ->
          ...
          let bindings =
            ...
            | Some arg_ty, Some p ->
                let concrete_ty =
                  freshen_arrow_effects level (subst_tgens all_fresh arg_ty)
                in
                check_pattern ctx level p concrete_ty
            | None, Some _ ->
                error ctx
                  (Printf.sprintf "constructor %s takes no arguments" name)
            | Some _, None ->
                error ctx
...
and check_existential_escape ctx existential_refs result_ty =
  ...
  let rec collect_tvar_refs acc ty =
    ...
    | Types.TArrow (a, _, b) | Types.TCont (a, _, b) ->
        collect_tvar_refs (collect_tvar_refs acc a) b
    ...
    | Types.TRecord row ->
        List.fold_left
          (fun a (_, t) -> collect_tvar_refs a t)
          acc
    ...
    | Types.TPolyVariant row ->
        let rec collect_pv a = function
          | Types.PVRow (_, Some t, tail) ->
              collect_pv (collect_tvar_refs a t) tail
      ...
      (fun exist_ref ->
        ...
        | resolved ->
         ...
         type is also in result_ty *)
            ...
            List.exists
              (fun er -> List.exists (fun rr -> er == rr) result_refs)
              exist_refs)
      existential_refs
  in
  if has_escape then
    error ctx
      "existential type variable would escape the scope of its GADT match arm"
...
and synth_match expected_ty ctx level scrut arms partial =
  ...
  | Some (_gadt_name, scrut_params) -> (
      ...
      let result_ty =
        match expected_ty with Some ty -> ty | None -> Types.new_tvar level
      in
      (* Extract tvar refs from scrutinee params for snapshot/restore *)
      let get_tvar_ref ty =
        match Types.repr ty with Types.TVar r -> Some r | _ -> None
      in
      let param_refs =
        List.filter_map
          (fun p ->
            match get_tvar_ref p with Some r -> Some (r, !r) | None -> None)
          scrut_params
      in
      (* Also save all params' current repr values for non-tvar params *)
      let save_snapshot () = List.map (fun (r, _) -> (r, !r)) param_refs in
      let restore_snapshot saved = List.iter (fun (r, v) -> r := v) saved in
      let check_gadt_arm (pat, guard, body) =
        ...
        let ctor_name =
          match pat with Ast.PatConstruct (name, _) -> Some name | _ -> None
        in
        let is_gadt_ctor =
        ...
        let bindings, existential_refs =
          if is_gadt_ctor then check_pattern_gadt ctx level pat scrut_te.ty
          else (check_pattern ctx level pat scrut_te.ty, [])
        in
        let ctx' =
          List.fold_left (fun c (n, t) -> extend_var_mono c n t) ctx bindings
        in
        let guard_te =
          match guard with
...
and check_pattern ctx level (pat : Ast.pattern) (ty : Types.ty) :
  ...
  | Ast.PatInt _ ->
      try_unify ctx ty Types.TInt;
      []
  | Ast.PatFloat _ ->
      try_unify ctx ty Types.TFloat;
      []
  | Ast.PatBool _ ->
      try_unify ctx ty Types.TBool;
      []
  | Ast.PatString _ ->
      try_unify ctx ty Types.TString;
      []
  | Ast.PatUnit ->
      try_unify ctx ty Types.TUnit;
      []
  | Ast.PatTuple pats ->
  ...
  | Ast.PatConstruct (name, arg_pat) -> (
      ...
      | Some info -> (
          ...
          then
            error ctx (Printf.sprintf "unknown constructor in pattern: %s" name);
          let fresh_args =
            List.init info.ctor_num_params (fun _ -> Types.new_tvar level)
          in
          let expected_variant =
            Types.TVariant (info.ctor_type_name, fresh_args)
          in
          ...
          | Some arg_ty, Some p ->
              let concrete_ty =
                freshen_arrow_effects level (subst_tgens fresh_args arg_ty)
              in
              check_pattern ctx level p concrete_ty
          | None, Some _ ->
              error ctx
        ...
        | _ -> (
            ...
            let candidates =
              List.filter
                (fun (_name, fields) ->
                  List.for_all
                    (fun fn -> List.mem_assoc fn fields)
                    pat_field_names)
                ctx.type_env.Types.records
            in
            match candidates with
                ...
                | Types.TVar _ ->
                    let row =
                      List.fold_right
                        ...
                        pat_field_names (Types.new_rvar level)
                    in
                    try_unify ctx ty (Types.TRecord row);
                    Types.record_row_to_fields
                      (match Types.repr ty with
                      | Types.TRecord r -> r
                      | _ ->
                          error ctx "record pattern used with non-record type")
      ...
      List.concat
        (List.map
           (fun (name, pat) ->
             ...
             | None ->
                 error ctx (Printf.sprintf "record type has no field %s" name))
           field_pats)
  ...
  | Ast.PatSet elems ->
      let elem_ty = Types.new_tvar level in
      try_unify ctx ty (Types.TVariant ("set", [ elem_ty ]));
      List.concat_map (fun pat -> check_pattern ctx level pat elem_ty) elems
  ...
  | Ast.PatPin name -> (
      ...
      | Some scheme ->
          ...
          []
      | None ->
          error ctx (Printf.sprintf "unbound variable in pin pattern: %s" name))
...
and synth_poly_rec_fn ctx level name qualified_name type_params params ret_annot
    body =
  let shared_tvars = Hashtbl.create 4 in
  let shared_eff_tvars = Hashtbl.create 2 in
  ...
  let ret_ty, body_eff =
    ...
    | Some (Ast.TyWithEffect (ty, eff_annot)) ->
        let ty' =
          resolve_ty_annot_shared ctx (level + 1) shared_tvars shared_eff_tvars
            ty
        in
        let eff =
          match eff_annot with
          | Ast.EffAnnotPure -> Types.EffEmpty
          | Ast.EffAnnotRow items ->
              resolve_eff_items ctx (level + 1) shared_tvars shared_eff_tvars
                items
        in
        (ty', eff)
    | Some annot ->
  ...
  let inner_ctx =
    List.fold_left2
      (fun c (p : Ast.param) ty -> extend_var_mono c p.name ty)
      { ctx_with_self with current_eff = body_eff }
      params param_tys
  in
...
let process_type_def ctx type_params type_name (def : Ast.type_def) : ctx =
  ...
  | Ast.TDVariant ctors ->
          ...
          type_env =
            {
              ctx.type_env with
              variants =
                (type_name, num_params, [], false) :: ctx.type_env.variants;
            };
        }
      in
      ...
      let resolve_with_params annot =
        let tvars = Hashtbl.create 4 in
        resolve_ty_annot_def ~loc:pre_ctx.loc ~check_synonyms:true
          pre_ctx.type_env
          (make_def_resolve_var param_tbl tvars)
          eff_empty annot
      in
      ...
      let rec collect_annot_vars acc = function
        ...
        | Ast.TyArrow (a, b, _) ->
            collect_annot_vars (collect_annot_vars acc a) b
        ...
        | Ast.TyRecord (fs, _) ->
            List.fold_left (fun a (_, t) -> collect_annot_vars a t) acc fs
        ...
        | Some ret ->
            ...
            let ret_type_name, ret_type_args =
              ...
              | _ ->
                  error ctx
                    (Printf.sprintf
                       "GADT constructor %s: return type must be %s" name
                       type_name)
            in
            let canonical_ret =
              resolve_type_alias pre_ctx.type_env ret_type_name
            in
            ...
            let name_matches =
              ...
              | Some i ->
                  let short =
                    String.sub type_name (i + 1)
                      (String.length type_name - i - 1)
                  in
            ...
            if not name_matches then
              error ctx
                (Printf.sprintf
                   "GADT constructor %s: return type must be %s, got %s" name
                   type_name ret_type_name);
            if List.length ret_type_args <> num_params then
              error ctx
                (Printf.sprintf
                   "GADT constructor %s: return type has %d type arg(s), \
                    expected %d"
                   name
                   (List.length ret_type_args)
                   num_params);
            ...
            List.iteri
              (fun i e -> Hashtbl.replace gadt_tbl e (num_params + i))
              existentials;
            (* 5. Resolve types using GADT param table *)
            let gadt_tvars = Hashtbl.create 4 in
            let gadt_resolve annot =
              resolve_ty_annot_def ~loc:pre_ctx.loc ~check_synonyms:true
                pre_ctx.type_env
                (make_def_resolve_var gadt_tbl gadt_tvars)
                eff_empty annot
            in
      ...
      let type_env =
        {
          ctx.type_env with
          variants =
            (type_name, num_params, variant_def, is_gadt)
            :: ctx.type_env.variants;
  ...
  | Ast.TDRecord fields ->
      ...
      in
      let sorted_fields =
        List.sort (fun (a, _) (b, _) -> String.compare a b) typed_fields
      in
      let type_env =
        {
          ctx.type_env with
         ...
         can be used in type annotations like (string, int) Hashtbl.t *)
          type_synonyms =
            ( type_name,
              num_params,
              Types.TRecord (Types.fields_to_closed_row sorted_fields) )
            :: ctx.type_env.type_synonyms;
        }
      in
      { ctx with type_env }
  | Ast.TDAlias annot ->
      ...
      let tvars = Hashtbl.create 4 in
      let resolved_ty =
        resolve_ty_annot_def ~loc:ctx.loc ~check_synonyms:true ctx.type_env
          (make_def_resolve_var param_tbl tvars)
          eff_empty annot
      in
      let type_env =
        {
          ctx.type_env with
          type_synonyms =
            (type_name, num_params, resolved_ty) :: ctx.type_env.type_synonyms;
        }
      in
      { ctx with type_env }
      ...
      let type_env =
        {
          ctx.type_env with
          variants =
            (type_name, num_params, variant_def, false) :: ctx.type_env.variants;
...
let wrap_params_decl params ret_annot body =
  ...
  | Some (Ast.TyWithEffect (ret_ty, eff_annot)) when params <> [] ->
      ...
      let full_fun =
        List.fold_right (fun param body -> Ast.EFun (param, body)) params body
      in
...
let process_class_def ctx (class_name : string) (tyvar_names : string list)
  ...
  if
    List.exists
      (fun c -> String.equal c.Types.class_name class_name)
      ctx.type_env.Types.classes
  then error ctx (Printf.sprintf "duplicate class definition: %s" class_name);
  ...
  let method_tys =
    List.map
      (fun (mname, annot) ->
        let tvars = Hashtbl.create 4 in
        List.iteri
          (fun i name -> Hashtbl.replace tvars name (Types.TGen i))
          tyvar_names;
          ...
          | Some (Ast.EffAnnotRow items) ->
              ...
              List.iter
                (fun item ->
                  ...
                  | Ast.EffVar name -> tail := resolve_eff_var name)
                items;
              List.fold_right
                (fun (name, params) acc -> Types.EffRow (name, params, acc))
                (List.rev !labels) !tail
        in
        let ty =
          resolve_ty_annot_def ~loc:ctx.loc ~check_synonyms:false ctx.type_env
            resolve_var class_resolve_eff annot
        in
      ...
      (fun (from_names, to_names) ->
        let resolve_idx name =
          ...
          | None ->
              error ctx
                (Printf.sprintf
                   "functional dependency type variable '%s not in class \
                    parameters"
                   name)
        in
        Types.
          {
            fd_from = List.map resolve_idx from_names;
  ...
  let ctx =
    List.fold_left
      (fun ctx (mname, mty, mquant, mequant) ->
        let scheme =
          {
            Types.quant = mquant;
            equant = mequant;
  ...
  in
  let type_env =
    { ctx.type_env with Types.classes = class_def :: ctx.type_env.classes }
  in
...
let resolve_inst_tys ctx (annots : Ast.ty_annot list) =
  ...
  let resolve annot =
    resolve_ty_annot_def ~loc:ctx.loc ~check_synonyms:false ctx.type_env
      resolve_var eff_empty annot
  in
  let tys = List.map resolve annots in
  (tys, !next_idx, tvars)

let process_instance_def ctx level (class_name : string)
    (inst_ty_annots : Ast.ty_annot list) (constraints : Ast.constraint_ list)
  ...
  let stored_tys, num_inst_vars, inst_tvars =
    resolve_inst_tys ctx inst_ty_annots
  in
  if List.length stored_tys <> num_class_params then
    error ctx
      (Printf.sprintf "class %s expects %d type parameters, got %d" class_name
         num_class_params (List.length stored_tys));
  let dname = Types.dict_name class_name stored_tys in
  (* Create fresh tvars for typechecking method bodies *)
  let check_tvar_list =
    List.init num_inst_vars (fun _ -> Types.new_tvar (level + 1))
  in
  let check_tys =
    List.map (fun t -> subst_tgens check_tvar_list t) stored_tys
  in
  if
    List.exists
      (fun i ->
        ...
        && Types.match_partial_inst i.Types.inst_tys
             (List.map (fun t -> Some t) stored_tys))
      ctx.type_env.Types.instances
  then
    error ctx
      (Printf.sprintf "duplicate instance %s for types %s" class_name
         (String.concat ", " (List.map Types.pp_ty stored_tys)));
  List.iter
    (fun (mname, _) ->
      if not (List.exists (fun (n, _, _) -> String.equal n mname) methods) then
        error ctx
          (Printf.sprintf "instance %s %s missing method: %s" class_name
             (String.concat " " (List.map Types.pp_ty stored_tys))
             mname))
    ...
    List.map
      (fun (mname, params, body) ->
        let method_schema_ty =
          ...
          | None ->
              error ctx
                (Printf.sprintf "%s is not a method of class %s" mname
                   class_name)
        in
            ...
            Types.instantiate (level + 1)
              {
                ...
                constraints = [];
                record_evidences = [];
                body = concrete_method_ty;
              }
          else concrete_method_ty
        in
  ...
  in
  let sorted_methods =
    List.sort (fun (a, _) (b, _) -> String.compare a b) typed_methods
  in
  let record_ty =
    Types.TRecord
      (Types.fields_to_closed_row
         (List.map (fun (n, te) -> (n, te.ty)) sorted_methods))
  in
          ...
          List.map
            (fun tvname ->
              ...
              | None ->
                  error ctx
                    (Printf.sprintf
                       "constraint type variable '%s not found in instance \
                        type parameters"
                       tvname))
            tyvar_names
        in
  ...
  (try
     Types.check_fundep_consistency class_def inst_def
       ctx.type_env.Types.instances
   with Types.Unify_error msg -> error ctx msg);
  let type_env =
    { ctx.type_env with Types.instances = inst_def :: ctx.type_env.instances }
  in
  let ctx = { ctx with type_env } in
...
let extract_tyvar_ids annot ty =
  ...
  let rec walk annot ty =
    ...
    | Ast.TyVar name, Types.TVar { contents = Types.Unbound (id, _) } ->
        if not (Hashtbl.mem mapping name) then Hashtbl.replace mapping name id
    | ( Ast.TyArrow (a1, a2, _),
        (Types.TArrow (t1, _, t2) | Types.TCont (t1, _, t2)) ) ->
        walk a1 t1;
        walk a2 t2
    ...
    | Ast.TyApp (args, _), Types.TVariant (_, tys)
      when List.length args = List.length tys ->
...
let resolve_let_constraints ~loc type_env level constraints params ret_annot
    te_ty shared_tvars =
  if constraints = [] then Types.generalize level te_ty
  else begin
    ...
    | Some annot ->
        let rec skip_arrows ty n =
          if n = 0 then ty
            ...
            match Types.repr ty with
            | Types.TArrow (_, _, rest) | Types.TCont (_, _, rest) ->
                skip_arrows rest (n - 1)
            | _ -> ty
        in
              ...
              (fun tvname ->
                ...
                | Some tvar_id -> (
                    ...
                    | _ ->
                        error_at loc
                          (Printf.sprintf
                             "constraint type variable '%s is not polymorphic"
                             tvname))
                | None -> (
                    ...
                    | Some tv -> (
                        ...
                        | _ ->
                            ...
                            let idx =
                              match Hashtbl.find_opt phantom_map tvname with
                              | Some idx -> idx
                              | None ->
                                  let idx = !phantom_counter in
                                  incr phantom_counter;
                    ...
                    | None ->
                        error_at loc
                          (Printf.sprintf
                             "constraint type variable '%s not found in \
                              function signature"
                             tvname)))
              tyvar_names
          in
...
let unify_constraint_tvars type_env constraints shared_tvars body_te =
  ...
  let rec walk te =
        ...
        | Some class_def -> (
            ...
            | Some method_schema_ty ->
                ...
                let rec go s r =
                  ...
                  match (s, r) with
                  | Types.TGen i, _ when i < num_params ->
                      if not (Hashtbl.mem param_map i) then
                        Hashtbl.replace param_map i r
                  | Types.TArrow (s1, _, s2), Types.TArrow (r1, _, r2)
                  | Types.TCont (s1, _, s2), Types.TCont (r1, _, r2) ->
                      go s1 r1;
                      go s2 r2
                ...
                List.iter
                  (fun (cclass, tyvar_names) ->
                    if
                      String.equal cclass class_def.Types.class_name
                      && List.length tyvar_names = num_params
                    then begin
                            ...
                            List.for_all
                              (fun fi ->
                                ...
                                | Some body_tv -> (
                                    ...
                                    match
                                      Hashtbl.find_opt shared_tvars tvname
                                    with
                                    | Some constraint_tv ->
                                        Types.repr body_tv
                                        == Types.repr constraint_tv
                                    | None -> true)
                                | None -> true)
                              fd.fd_from)
                          class_def.Types.class_fundeps
                            ...
                            List.iter
                              (fun ti ->
                                ...
                                | Some body_tv -> (
                                    ...
                                    with
                                    | Some constraint_tv -> (
                                        try Types.unify body_tv constraint_tv
                                        with Types.Unify_error _ -> ())
                                    | None -> ())
                                | None -> ())
                              fd.fd_to)
    ...
    | TEMatch (scrut, branches, _) ->
        ...
        List.iter
          (fun (_, guard, body) ->
            (match guard with Some g -> walk g | None -> ());
            walk body)
          branches
    | TESeq (e1, e2) ->
        walk e1;
    ...
    | TELetRecAnd (bindings, body) ->
        List.iter (fun (_, e) -> walk e) bindings;
        walk body
    | _ -> ()
    (* TEInt, TEFloat, TEBool, TEString, TEByte, TERune, TEUnit, TENil, TEContinueLoop, TEConstruct None *)
  in
  walk body_te
...
let synth_constrained_fn ctx level constraints params ret_annot body =
  ...
  let inner_ctx =
    List.fold_left2
      ...
      {
        ctx with
        constraint_tvars = constraint_tvar_ids @ ctx.constraint_tvars;
        current_eff = body_eff;
      }
      params param_tys
  in
  ...
  in
  let scheme =
    resolve_let_constraints ~loc:ctx.loc ctx.type_env level constraints params
      ret_annot te.ty shared_tvars
  in
  (te, scheme)
...
let open_module_into_ctx ctx mod_name names_opt =
  ...
  | Some minfo ->
        ...
        List.fold_left
          (fun te (short, info) ->
            if filter short then
              {
                te with
                Types.constructors = (short, info) :: te.Types.constructors;
              }
            else te)
          ctx.type_env minfo.Types.mod_pub_constructors
      in
          ...
          (fun te qname ->
            ...
            if filter short then begin
              ...
              let te =
                match
                  List.find_opt
                    (fun (n, _, _, _) -> String.equal n qname)
                    te.Types.variants
                with
                | Some (_, np, vdef, is_gadt) ->
                    if
                      List.exists
                        (fun (n, _, _, _) -> String.equal n short)
                        te.variants
                    then te
                    else
                      {
                        te with
                        variants = (short, np, vdef, is_gadt) :: te.variants;
                      }
                | None -> te
              in
              let te =
                match List.assoc_opt qname te.Types.records with
                | Some fields ->
              ...
              let te =
                match
                  List.find_opt
                    (fun (n, _, _) -> String.equal n qname)
                    te.Types.type_synonyms
                with
                | Some (_, np, expanded) ->
                    if
                      List.exists
                        (fun (n, _, _) -> String.equal n short)
                        te.type_synonyms
                    then te
                    else
                      {
                        te with
                        type_synonyms =
                          (short, np, expanded) :: te.type_synonyms;
                      }
                | None -> te
              in
              (* Add type alias so short name resolves to qualified name *)
              let te =
                if String.equal short qname then te
                else
                  {
                    te with
                    Types.type_aliases = (short, qname) :: te.Types.type_aliases;
                  }
              in
              te
            end
            else te)
          type_env minfo.Types.mod_pub_types
      in
      (* Import newtypes *)
      let type_env =
        List.fold_left
      ...
      let type_env =
        List.fold_left
          (fun te (sub_name, sub_info) ->
            if filter sub_name then
              {
                te with
                Types.modules = (sub_name, sub_info) :: te.Types.modules;
              }
            else te)
          type_env minfo.Types.mod_submodules
      in
      ...
      let type_env =
        List.fold_left
          ...
          type_env minfo.Types.mod_pub_classes
      in
      { ctx with type_env }
...
let rec collect_tyvars_annot = function
  ...
  | Ast.TyRecord (fs, _) ->
      List.concat_map (fun (_, t) -> collect_tyvars_annot t) fs
...
let gen_show_variant type_params name ctors =
      ...
      (fun (ctor_name, arg_annot, _) ->
        ...
        | Some (Ast.TyTuple tys) ->
            ...
            let pat =
              Ast.PatConstruct
                ( ctor_name,
                  Some (Ast.PatTuple (List.map (fun v -> Ast.PatVar v) vars)) )
            in
            let parts =
  ...
  ( "Show",
    ...
    [
      ( "show",
        [ Ast.{ name = "__x"; annot = None; is_generated = false } ],
        body );
    ] )
...
let gen_show_record type_params name fields =
  ...
  let parts =
    ...
    @ List.concat
        (List.mapi
           (fun i (f : Ast.record_field_def) ->
             let sf =
               Ast.EApp
                 (Ast.EVar "show", Ast.EField (Ast.EVar "__x", f.rfd_name))
             in
...
let gen_eq_variant type_params name ctors =
  ...
  let and_exprs = function
    ...
    | [ e ] -> e
    | first :: rest ->
        List.fold_left (fun a e -> Ast.EBinop (Ast.And, a, e)) first rest
  in
  let arms =
    List.map
      (fun (ctor_name, arg_annot, _) ->
        ...
        | Some (Ast.TyTuple tys) ->
            ...
            let lp =
              Ast.PatConstruct
                ( ctor_name,
                  Some (Ast.PatTuple (List.map (fun v -> Ast.PatVar v) vs)) )
            in
            let rp =
              Ast.PatConstruct
                ( ctor_name,
                  Some (Ast.PatTuple (List.map (fun v -> Ast.PatVar v) ws)) )
            in
            let eqs =
              List.map2
                (fun v w -> Ast.EBinop (Ast.Eq, Ast.EVar v, Ast.EVar w))
                vs ws
            in
  ...
  let eq_body =
    Ast.EMatch (Ast.ETuple [ Ast.EVar "__a"; Ast.EVar "__b" ], arms, Total)
  in
  let neq_body =
    Ast.EUnop
      ( Ast.Not,
        Ast.EMatch
          ( Ast.ETuple [ Ast.EVar "__a"; Ast.EVar "__b" ],
            List.map (fun (p, g, e) -> (p, g, e)) arms,
            Total ) )
  in
  ( "Eq",
    [ inst_ty ],
    ...
    [
      ( "=",
        [
          Ast.{ name = "__a"; annot = None; is_generated = false };
          Ast.{ name = "__b"; annot = None; is_generated = false };
        ],
        eq_body );
      ( "<>",
        [
...
let generate_derived_instance ~loc type_params name def class_name =
  ...
  | _, Ast.TDVariant ctors
    when List.exists (fun (_, _, ret) -> ret <> None) ctors ->
      error_at loc
        (Printf.sprintf "cannot derive %s for GADT type %s" class_name name)
  | "Show", Ast.TDVariant ctors ->
      Some (gen_show_variant type_params name ctors)
  | "Show", Ast.TDRecord fields ->
      Some (gen_show_record type_params name fields)
  | "Show", Ast.TDNewtype (ctor_name, underlying) ->
      Some
        (gen_show_variant type_params name
           [ (ctor_name, Some underlying, None) ])
  ...
  | "Eq", Ast.TDNewtype (ctor_name, underlying) ->
      Some
        (gen_eq_variant type_params name [ (ctor_name, Some underlying, None) ])
  | _ -> None
...
let rec process_module_def ctx level mod_name (items : Ast.module_decl list) =
  ...
  let sub_ctx =
    List.fold_left
      (fun sub_ctx (item : Ast.module_decl) ->
    ...
    List.filter
      (fun (name, _, _, _) ->
        not
          (List.exists
             (fun (n, _, _, _) -> String.equal n name)
             ctx.type_env.Types.variants))
      sub_ctx.type_env.Types.variants
  in
  let new_records =
    List.filter
      (fun (name, _) ->
        not
          (List.exists
             ...
             ctx.type_env.Types.records))
      sub_ctx.type_env.Types.records
  in
  let new_synonyms =
    List.filter
      (fun (name, _, _) ->
        not
          (List.exists
             ...
             ctx.type_env.Types.type_synonyms))
      sub_ctx.type_env.Types.type_synonyms
  in
  ...
  in
  let submod_type_names =
    List.concat_map (fun (_, info) -> submodule_types info) !(acc.ma_submodules)
  in
  let is_exported name =
    List.mem name !(acc.ma_pub_types) || List.mem name submod_type_names
  in
  let private_type_names =
    List.filter_map
      (fun (name, _, _, _) -> if is_exported name then None else Some name)
      new_variants
    @ List.filter_map
        (fun (name, _) -> if is_exported name then None else Some name)
        new_records
    @ List.filter_map
        (fun (name, _, _) -> if is_exported name then None else Some name)
        new_synonyms
  in
  let outer_type_env =
    {
      outer_type_env with
      Types.hidden_types =
        private_type_names @ outer_type_env.Types.hidden_types;
    }
  in
  ...
  let outer_type_env =
    {
      outer_type_env with
      Types.hidden_ctor_types =
        hidden_ctor_type_names @ outer_type_env.Types.hidden_ctor_types;
    }
  in
  ...
  let rec collect_type_names acc ty =
    match Types.repr ty with
    | Types.TVariant (name, args) ->
        List.fold_left collect_type_names (name :: acc) args
  ...
  and collect_row_names acc = function
    | Types.RRow (_, ty, rest) ->
        collect_row_names (collect_type_names acc ty) rest
    | _ -> acc
  in
        ...
        List.iter
          (fun tname ->
            if List.mem tname private_type_names then begin
              ...
              error ctx
                (Printf.sprintf
                   "public binding '%s' exposes private type '%s'. Use 'pub \
                    type' or 'opaque type' to export it"
                   short short_tname)
            end)
          type_names)
      !(acc.ma_pub_vars);
  ...
  let outer_type_env =
    List.fold_left
      (fun te (short, info) ->
        ...
        {
          te with
          Types.constructors = (qualified, info) :: te.Types.constructors;
        })
      outer_type_env !(acc.ma_pub_constructors)
  in
  (* Add instances to outer env — deduplicate since sub_ctx already has them *)
  let existing_dict_names =
    List.map (fun i -> i.Types.inst_dict_name) outer_type_env.Types.instances
  in
  let new_insts =
    List.filter
      (fun inst -> not (List.mem inst.Types.inst_dict_name existing_dict_names))
      !(acc.ma_all_instances)
  in
  let outer_type_env =
    {
      outer_type_env with
  ...
  let outer_mutable_vars =
    List.fold_left
      (fun mvs short ->
        let qualified = prefix ^ short in
        qualified :: mvs)
      ctx.mutable_vars !(acc.ma_pub_mutable_vars)
  in
  let outer_ctx =
    {
      ctx with
  ...
  ( outer_ctx,
    TDModule
      (mod_name, List.rev !(acc.ma_typed_decls), !(acc.ma_all_member_schemes))
  )
...
and check_module_item sub_ctx level prefix (item : Ast.module_decl)
  ...
  | Ast.DModule (inner_name, inner_items) ->
      let sub_ctx', tdecl =
        process_module_def sub_ctx level inner_name inner_items
      in
  ...
  | Ast.DType
          ...
          type_env =
            {
              sub_ctx.type_env with
              Types.type_aliases =
                (name, qualified_name) :: sub_ctx.type_env.Types.type_aliases;
            };
        }
      in
      (* Process type under qualified name *)
      let sub_ctx' =
        process_type_def sub_ctx_with_alias type_params qualified_name def
      in
      (* Track newtypes *)
      (match def with
      | Ast.TDNewtype _ ->
      ...
      | Ast.Public -> (
          ...
          | Ast.TDVariant ctors ->
              List.iter
                (fun (ctor_name, _, _) ->
                  match
                    List.assoc_opt ctor_name
                      sub_ctx'.type_env.Types.constructors
                  with
                  | Some info ->
                      let qual_info =
                        { info with Types.ctor_type_name = qualified_name }
                      in
                      acc.ma_pub_constructors :=
                        (ctor_name, qual_info) :: !(acc.ma_pub_constructors)
                  | None -> ())
                ctors
          | Ast.TDNewtype (ctor_name, _) -> (
              match
                List.assoc_opt ctor_name sub_ctx'.type_env.Types.constructors
              with
              | Some info ->
                  let qual_info =
                    { info with Types.ctor_type_name = qualified_name }
      ...
      let sub_ctx' =
        List.fold_left
          (fun ctx cls ->
            match
              generate_derived_instance ~loc:sub_ctx.loc type_params name def
                cls
            with
            | Some (class_name, inst_tys, constraints, methods) ->
                let ctx', tdecl =
                  process_instance_def ctx level class_name inst_tys constraints
                    methods
                in
                acc.ma_typed_decls := tdecl :: !(acc.ma_typed_decls);
            ...
            | None ->
                error sub_ctx
                  (Printf.sprintf "cannot derive %s for type %s" cls name))
          sub_ctx' deriving
      in
  ...
  | Ast.DLet { name; params; ret_annot; constraints; body } ->
      ...
      let te, scheme =
        if constraints <> [] then
          ...
          let scheme =
            if params = [] && not (is_syntactic_value body) then
              Types.mono te.ty
            else
              resolve_let_constraints ~loc:sub_ctx.loc Types.empty_type_env
                level [] params ret_annot te.ty (Hashtbl.create 0)
          in
          let scheme =
            infer_implicit_constraints "" sub_ctx.type_env sub_ctx.vars te
              scheme
          in
      ...
      acc.ma_all_member_schemes :=
        (qualified_name, scheme) :: !(acc.ma_all_member_schemes);
      (match item.vis with
  ...
  | Ast.DLetRec
      ...
      let fn_te, scheme =
        if type_params <> [] then
          ...
          let sub_ctx_self =
            extend_var_mono sub_ctx_self qualified_name fn_var
          in
          let te, scheme =
            synth_constrained_fn sub_ctx_self level constraints params ret_annot
              body
          in
          ...
          let scheme =
            resolve_let_constraints ~loc:sub_ctx.loc Types.empty_type_env level
              [] params ret_annot fn_te.ty (Hashtbl.create 0)
          in
          let scheme =
            infer_implicit_constraints name sub_ctx.type_env sub_ctx.vars fn_te
              scheme
          in
  ...
  | Ast.DClass { class_name = name; tyvars; fundeps; methods } ->
      ...
      let sub_ctx', tdecl =
        process_class_def sub_ctx qualified tyvars fundeps methods
      in
          ...
          type_env =
            {
              sub_ctx'.type_env with
              Types.type_aliases =
                (name, qualified) :: sub_ctx'.type_env.Types.type_aliases;
            };
        }
      in
      (match item.vis with
      | Ast.Public ->
          ...
          List.iter
            (fun (mname, _) ->
              ...
              | Some scheme ->
                  acc.ma_pub_vars := (mname, scheme) :: !(acc.ma_pub_vars)
              | None -> ())
            methods
      | _ -> ());
      (sub_ctx', tdecl)
  | Ast.DInstance
      {
        inst_class = class_name;
        inst_types = inst_ty_annots;
        inst_constraints = constraints;
        inst_methods = methods;
      } ->
      let sub_ctx', tdecl =
        process_instance_def sub_ctx level class_name inst_ty_annots constraints
          methods
      in
        ...
        List.filter
          (fun inst ->
            not
              (List.exists
                 (fun prev ->
                   String.equal prev.Types.inst_dict_name
  ...
  | Ast.DEffect (name, type_params, ops) ->
      ...
      let type_env =
        {
          sub_ctx.type_env with
          effects = effect_def :: sub_ctx.type_env.effects;
        }
      in
  ...
  | Ast.DExpr expr ->
      ...
      let rec close_ambient_effects e =
        ...
        | remaining -> (
            ...
            with Types.Unify_error _ ->
              error sub_ctx
                (Printf.sprintf "expression has unhandled effects: %s"
                   (Types.pp_eff eff)))
      in
  ...
  | Ast.DOpen (mod_name, names_opt) ->
      ...
      let minfo =
        ...
        | Some m -> m
        | None -> error sub_ctx (Printf.sprintf "unknown module: %s" mod_name)
      in
          ...
          (fun qname ->
            match
              List.find_opt
                ...
                sub_ctx.type_env.Types.classes
            with
      ...
      let sub_ctx' =
        List.fold_left
          ...
          sub_ctx' minfo.Types.mod_pub_vars
      in
  ...
  | Ast.DLetRecAnd bindings ->
      ...
      if has_poly then begin
        let shared_tvars_list =
          List.map
            (fun { Ast.type_params; params; ret_annot; _ } ->
              let shared_tvars = Hashtbl.create 4 in
              let shared_eff_tvars = Hashtbl.create 2 in
              List.iter
                (fun tp ->
              ...
              let param_tys =
                List.map
                  (fun (p : Ast.param) ->
                    match p.annot with
                    | Some annot ->
                        resolve_ty_annot_shared sub_ctx (level + 1) shared_tvars
                          shared_eff_tvars annot
                    | None -> Types.new_tvar (level + 1))
                  params
              in
              let ret_ty, body_eff =
                ...
                | Some (Ast.TyWithEffect (ty, eff_annot)) ->
                    ...
                    let eff =
                      ...
                      | Ast.EffAnnotPure -> Types.EffEmpty
                      | Ast.EffAnnotRow items ->
                          resolve_eff_items sub_ctx (level + 1) shared_tvars
                            shared_eff_tvars items
                    in
                    (ty', eff)
                | Some annot ->
        ...
        let schemes =
          List.map
            (fun (_, _, _, _, fn_ty) -> Types.generalize level fn_ty)
            shared_tvars_list
        in
        (* Bind both short and qualified names *)
        let sub_ctx' =
          List.fold_left2
            (fun ctx { Ast.lr_name = name; _ } scheme ->
              let qualified_name = prefix ^ name in
              let ctx = extend_var ctx name scheme in
              extend_var ctx qualified_name scheme)
            sub_ctx bindings schemes
        in
        let body_tes =
          List.map2
            (fun { Ast.params; body; _ } (_, param_tys, ret_ty, body_eff, _) ->
              let inner_ctx =
                List.fold_left2
                  (fun c (p : Ast.param) ty -> extend_var_mono c p.name ty)
                  { sub_ctx' with current_eff = body_eff }
                  params param_tys
              in
              let body_te = check inner_ctx (level + 1) body ret_ty in
              match (List.rev params, List.rev param_tys) with
        ...
        (* Track visibility *)
        List.iter
          (fun { Ast.lr_name = name; _ } ->
            let scheme =
              List.assoc name
                (List.map2
                   (fun { Ast.lr_name = n; _ } s -> (n, s))
                   bindings schemes)
            in
            match item.vis with
            | Ast.Public ->
        ...
        let body_tes =
          List.map2
            (fun { Ast.lr_name = _; params; ret_annot; constraints; body; _ }
                 (_, tv) ->
              if constraints <> [] then (
                let te, _scheme =
                  synth_constrained_fn sub_ctx' level constraints params
                    ret_annot body
                in
        ...
        let sub_ctx'' =
          List.fold_left2
            (fun ctx { Ast.lr_name = name; _ } (_, tv) ->
              let qualified_name = prefix ^ name in
              let scheme = Types.generalize level tv in
              let ctx = extend_var ctx name scheme in
              extend_var ctx qualified_name scheme)
            sub_ctx bindings fn_vars
        in
        let typed_bindings =
          List.map2
            (fun { Ast.lr_name = name; _ } te ->
  ...
  | Ast.DTypeAnd type_defs ->
      ...
      let sub_ctx' =
        List.fold_left
          (fun ctx
               { Ast.td_params = type_params; td_name = name; td_def = def; _ }
             ->
            let qualified_name = prefix ^ name in
            let num_params = List.length type_params in
            let ctx =
                ...
                type_env =
                  {
                    ctx.type_env with
                    Types.type_aliases =
                      (name, qualified_name) :: ctx.type_env.Types.type_aliases;
                  };
              }
            in
            match def with
                  ...
                  type_env =
                    {
                      ctx.type_env with
                      variants =
                        (qualified_name, num_params, [], false)
                        :: ctx.type_env.variants;
                    };
                }
            | Ast.TDRecord _ ->
      ...
      (* Visibility *)
      List.iter
        (fun { Ast.td_name = name; td_def = def; _ } ->
          let qualified_name = prefix ^ name in
          match item.vis with
          | Ast.Public -> (
              acc.ma_pub_types := qualified_name :: !(acc.ma_pub_types);
        ...
        List.fold_left
          (fun ctx
            ...
            List.fold_left
              (fun ctx cls ->
                match
                  generate_derived_instance ~loc:sub_ctx.loc type_params name
                    def cls
                with
                | Some (class_name, inst_tys, constraints, methods) ->
                    let ctx', tdecl =
                      process_instance_def ctx level class_name inst_tys
                        constraints methods
                    in
                    acc.ma_typed_decls := tdecl :: !(acc.ma_typed_decls);
                    acc.ma_all_instances := ctx'.type_env.Types.instances;
      ...
      let rec push_all_but_last = function
        ...
        | [ last ] -> last
        | hd :: tl ->
            acc.ma_typed_decls := hd :: !(acc.ma_typed_decls);
...
let check_decl ctx level (decl : Ast.decl) : ctx * tdecl list =
  ...
  | Ast.DType
      ...
      let ctx', derived_tdecls =
        List.fold_left
          (fun (ctx, acc) cls ->
            match
              generate_derived_instance ~loc:ctx.loc type_params name def cls
            with
            | Some (class_name, inst_tys, constraints, methods) ->
                let ctx', tdecl =
                  process_instance_def ctx level class_name inst_tys constraints
  ...
  | Ast.DLet { name; params; ret_annot; constraints; body } ->
      if constraints <> [] then begin
        let te, scheme =
          synth_constrained_fn ctx level constraints params ret_annot body
        in
        let ctx' = extend_var ctx name scheme in
        (ctx', [ TDLet (name, te) ])
      end
        ...
        let scheme =
          if params = [] && not (is_syntactic_value body) then Types.mono te.ty
            ...
            resolve_let_constraints ~loc:ctx.loc Types.empty_type_env level []
              params ret_annot te.ty (Hashtbl.create 0)
        in
        let scheme =
          infer_implicit_constraints "" ctx.type_env ctx.vars te scheme
        in
  ...
  | Ast.DLetRec
      ...
      if type_params <> [] then begin
        ...
        let te, scheme =
          synth_constrained_fn ctx_with_self level constraints params ret_annot
            body
        in
        ...
        if params = [] then begin
          ...
          | _ ->
              error ctx
                "let rec binding must be a function or constructive expression"
        end;
        let eff = Types.new_effvar level in
        let eff_ctx = { ctx with current_eff = eff } in
        let full_body = wrap_params_decl params ret_annot body in
        ...
        in
        let scheme =
          infer_implicit_constraints name ctx.type_env ctx.vars fn_te scheme
        in
  ...
  | Ast.DInstance
      ...
      let ctx', td =
        process_instance_def ctx level class_name inst_ty_annots constraints
          methods
      in
      (ctx', [ td ])
  | Ast.DEffect (name, type_params, ops) ->
      ...
      in
      let type_env =
        { ctx.type_env with effects = effect_def :: ctx.type_env.effects }
      in
      let ctx = { ctx with type_env } in
      (ctx, [ TDEffect name ])
  | Ast.DExtern (name, ty_annot) ->
  ...
  | Ast.DLetRecAnd bindings ->
      ...
      if has_poly then begin
        ...
        let typed_bindings =
          List.map2
            (fun { Ast.lr_name = name; _ } te -> (name, te))
            bindings body_tes
        in
        ...
        List.iter
          (fun { Ast.lr_name = name; params; body; _ } ->
            if params = [] then begin
              match strip_loc body with
              | Ast.EFun _ | Ast.EAnnot (Ast.EFun _, _) -> ()
              | _
                when is_constructive body
                     && not (is_immediately_linked name body) ->
                  ()
              | _ ->
                  error ctx
                    (Printf.sprintf
                       "let rec binding for '%s' must be a function or \
        ...
        let body_tes =
          List.map2
            (fun { Ast.lr_name = _; params; ret_annot; constraints; body; _ }
              ...
              if constraints <> [] then (
                let te, _scheme =
                  synth_constrained_fn ctx' level constraints params ret_annot
                    body
                in
                try_unify ctx tv te.ty;
                te)
  ...
  | Ast.DTypeAnd type_defs ->
                ...
                let type_env =
                  {
                    ctx.type_env with
                    variants =
                      (name, num_params, [], false) :: ctx.type_env.variants;
                  }
                in
                { ctx with type_env }
            | Ast.TDRecord _ ->
          ...
          (fun (ctx, acc)
            ...
            let ctx', new_decls =
              List.fold_left
                (fun (ctx, acc) cls ->
                  match
                    generate_derived_instance ~loc:ctx.loc type_params name def
                      cls
                  with
                  | Some (class_name, inst_tys, constraints, methods) ->
                      let ctx', tdecl =
      ...
      let type_decls =
        List.map
          (fun { Ast.td_name = name; td_def = def; _ } -> TDType (name, def))
          type_defs
      in
...
let build_tgen_map schema_ty actual_ty =
  ...
  let rec walk s a =
    let a = Types.repr a in
    match s with
    | Types.TGen i -> if not (Hashtbl.mem map i) then Hashtbl.replace map i a
    | Types.TArrow (s1, _, s2) -> (
        match a with
        | Types.TArrow (a1, _, a2) ->
            walk s1 a1;
...
let find_constrained_dict xctx tvar_id class_name =
  List.find_map
    (fun (tid, dparam, cls) ->
      if tid = tvar_id && String.equal cls class_name then Some dparam else None)
    xctx.xf_constrained
...
let rec resolve_dict_arg xctx (cc : Types.class_constraint) tgen_map =
  let arg_types =
    List.map
      (fun (ca : Types.class_arg) ->
        ...
        | CAWild ->
            Some (Types.new_tvar 1)
            (* fresh tvar, resolved by fundep improvement below *)
        | CAPhantom (idx, _) -> (
  ...
  if has_unresolved then begin
    if String.equal cc.cc_class "Show" then
      mk_xf xctx
        (TERecord [ ("show", mk_xf xctx (TEVar "__show_value") Types.TUnit) ])
        Types.TUnit
    else
      error_xf xctx
        (Printf.sprintf "could not resolve constraint type argument for %s"
           cc.cc_class)
  end
  else
    ...
    let arg_types =
      ...
      if has_unbound then
        ...
        | Some class_def ->
            ...
            in
            let improved =
              Types.improve_with_fundeps xctx.xf_type_env.Types.instances
                class_def partial
            in
            ...
            List.iter2
              (fun orig_ty imp ->
                ...
                | ( Some resolved,
                    Types.TVar ({ contents = Types.Unbound _ } as r) ) -> (
                    try Types.unify (Types.TVar r) resolved
                    with Types.Unify_error _ -> ())
                | _ -> ())
              arg_types improved;
    ...
    | None ->
        ...
        if has_unbound_tvar then begin
          if String.equal cc.cc_class "Show" then
            mk_xf xctx
              (TERecord
                 [ ("show", mk_xf xctx (TEVar "__show_value") Types.TUnit) ])
              Types.TUnit
          else
            error_xf xctx
              (Printf.sprintf "no instance of %s for types %s" cc.cc_class
                 (String.concat ", " (List.map Types.pp_ty arg_types)))
        end
        else begin
          ...
          | [ inst ] ->
              ...
              else mk_xf xctx (TEVar inst.inst_dict_name) Types.TUnit
          | [] ->
              let is_structural =
                List.exists
                  (fun ty ->
              ...
              if is_structural && String.equal cc.cc_class "Show" then
                let show_ty =
                  Types.TArrow (List.hd arg_types, Types.EffEmpty, Types.TString)
                in
                mk_xf xctx
                  (TERecord
                     [ ("show", mk_xf xctx (TEVar "__show_value") show_ty) ])
                  Types.TUnit
                ...
                mk_xf xctx
                  (TERecord
                     [
                       ("=", mk_xf xctx (TEVar "__structural_eq") Types.TUnit);
                       ("<>", mk_xf xctx (TEVar "__structural_neq") Types.TUnit);
                     ])
                  Types.TUnit
                ...
                mk_xf xctx
                  (TERecord
                     [
                       ("<", mk_xf xctx (TEVar "__structural_lt") Types.TUnit);
                       (">", mk_xf xctx (TEVar "__structural_gt") Types.TUnit);
                       ("<=", mk_xf xctx (TEVar "__structural_le") Types.TUnit);
                       (">=", mk_xf xctx (TEVar "__structural_ge") Types.TUnit);
                     ])
                  Types.TUnit
              else
                error_xf xctx
          ...
          | _ -> (
              ...
              | None ->
                  error_xf xctx
                    (Printf.sprintf "ambiguous instance for %s" cc.cc_class))
        end
...
let rec pat_bound_names = function
  ...
  | Ast.PatTuple pats | Ast.PatArray pats ->
      List.concat_map pat_bound_names pats
  | Ast.PatCons (p1, p2) -> pat_bound_names p1 @ pat_bound_names p2
  | Ast.PatConstruct (_, Some p) | Ast.PatPolyVariant (_, Some p) ->
      pat_bound_names p
  | Ast.PatRecord fields ->
      List.concat_map (fun (_, p) -> pat_bound_names p) fields
  ...
  | Ast.PatSet elems -> List.concat_map pat_bound_names elems
...
let rec xform_expr xctx te =
  ...
  | TEVar name ->
      ...
      if te' != te then te' else xform_constrained_ref xctx name te
  | TEBinop (op, e1, e2) ->
  ...
  | TELet (name, Some scheme, e1, e2) ->
      ...
      let xctx' =
        { xctx with xf_schemes = (name, scheme) :: xctx.xf_schemes }
      in
  ...
  | TELet (name, None, e1, e2) ->
      ...
      mk_xf xctx
        (TELet (name, None, xform_expr xctx e1, xform_expr xctx' e2))
        te.ty
  ...
  | TELetRec (name, None, e1, e2) ->
      let xctx' = { xctx with xf_locals = name :: xctx.xf_locals } in
      mk_xf xctx
        (TELetRec (name, None, xform_expr xctx' e1, xform_expr xctx' e2))
        te.ty
  | TELetMut (name, e1, e2) ->
      let xctx' = { xctx with xf_locals = name :: xctx.xf_locals } in
      mk_xf xctx
  ...
  | TELetRecAnd (bindings, body) ->
      ...
      let bindings' =
        List.map (fun (n, e) -> (n, xform_expr xctx' e)) bindings
      in
      mk_xf xctx (TELetRecAnd (bindings', xform_expr xctx' body)) te.ty
  | TEIf (c, t, e) ->
      mk_xf xctx
        (TEIf (xform_expr xctx c, xform_expr xctx t, xform_expr xctx e))
        te.ty
  | TETuple es -> mk_xf xctx (TETuple (List.map (xform_expr xctx) es)) te.ty
  | TERecord fields ->
      mk_xf xctx
        (TERecord (List.map (fun (n, e) -> (n, xform_expr xctx e)) fields))
        te.ty
  | TERecordUpdate (base, overrides) ->
      let base' = xform_expr xctx base in
      let overrides' =
        List.map (fun (n, e) -> (n, xform_expr xctx e)) overrides
      in
          ...
          List.map
            (fun (name, e) ->
              let ev_param =
                List.find_map
                  (fun (rid, fname, pname) ->
                    if rid = rvar_id && String.equal fname name then Some pname
                    else None)
                  xctx.xf_record_evidence
              in
              match ev_param with
              ...
              | None ->
                  error_xf xctx
                    (Printf.sprintf "missing record evidence for field %s" name))
            overrides'
        in
        mk_xf xctx (TERecordUpdateIdx (base', idx_val_pairs)) te.ty
      end
      else mk_xf xctx (TERecordUpdate (base', overrides')) te.ty
  | TERecordUpdateIdx (base, pairs) ->
      mk_xf xctx
        (TERecordUpdateIdx
           ( xform_expr xctx base,
             List.map
               (fun (i, v) -> (xform_expr xctx i, xform_expr xctx v))
               pairs ))
        te.ty
  ...
  | TEAssign (name, e) -> mk_xf xctx (TEAssign (name, xform_expr xctx e)) te.ty
  | TEFieldAssign (r, f, e) ->
      ...
      let arms' =
        List.map
          (fun arm ->
            ...
            | THOp { op_name; arg; k; body } ->
                let xctx' =
                  { xctx with xf_locals = k :: arg :: xctx.xf_locals }
                in
  ...
  | TEArray es -> mk_xf xctx (TEArray (List.map (xform_expr xctx) es)) te.ty
  | TEIndex (base, idx) ->
...
and xform_class_method xctx name te =
  ...
  if List.mem name xctx.xf_locals then te
  else
    ...
    let class_opt, method_name =
      ...
      | None -> (
          ...
          | Some i -> (
              ...
              let class_prefix = String.sub name 0 i in
              match
                Types.find_method_class xctx.xf_type_env.Types.classes short
              with
              | Some class_def
                when String.equal class_def.Types.class_name class_prefix ->
                  (Some class_def, short)
       ...
       a user-defined override will have a different body. *)
        ...
        if is_overridden then te
         ...
         Only if there's exactly one matching constraint to avoid ambiguity. *)
              ...
              | None ->
                  let has_unbound_arg =
                    List.exists
                      (fun i ->
                        match Hashtbl.find_opt found i with
                        | Some (Types.TVar { contents = Types.Unbound _ }) ->
                            true
                        | _ -> false)
                      (List.init num_params Fun.id)
                  in
          ...
          | Some dp ->
              mk_xf xctx
                (TEField (mk_xf xctx (TEVar dp) Types.TUnit, method_name))
                te.ty
          | None ->
              ...
              in
              let has_info =
                List.exists (fun opt -> opt <> None) partial_args
              in
              if has_info then begin
                ...
                in
                let all_concrete =
                  List.for_all (fun opt -> opt <> None) partial_args
                in
                let resolved =
                  match matching with
                  | [ inst ] -> Some inst
            ...
            where the original te.ty may have unresolved type variables. *)
                        ...
                        List.fold_left
                          (fun ty (i, inst_ty) ->
                            let rec subst t =
                              ...
                              | Types.TArrow (a, e, r) ->
                                  Types.TArrow (subst a, e, subst r)
                              | Types.TTuple ts ->
                                  Types.TTuple (List.map subst ts)
                            ...
                            subst ty)
                          method_ty
                          (List.mapi (fun i t -> (i, t)) inst.Types.inst_tys)
                      in
                      concrete_ty
                  | None -> te.ty
                in
                ...
                | Some inst when inst.inst_constraints <> [] ->
                    ...
                    let arg_types =
                      if all_concrete then List.filter_map Fun.id type_args
                      else
                  ...
                  and 'k=string but not 'v, we can extract 'v=int from the map type. *)
                          ...
                          (fun inst_ty arg_opt ->
                            ...
                            | Some concrete_ty ->
                                let rec walk s a =
                                  ...
                                  match s with
                                  | Types.TGen i ->
                                      if not (Hashtbl.mem inst_sub i) then
                                        Hashtbl.replace inst_sub i a
                                  | Types.TVariant (_, ss) -> (
                                      match a with
                                      | Types.TVariant (_, aa)
                        ...
                        let rec subst t =
                          match t with
                          | Types.TGen i -> (
                              match Hashtbl.find_opt inst_sub i with
                              | Some ty -> ty
                              | None -> t)
                          | Types.TArrow (a, e, r) ->
                              Types.TArrow (subst a, e, subst r)
                          ...
                          | Types.TArray t -> Types.TArray (subst t)
                          | Types.TVariant (n, ts) ->
                              Types.TVariant (n, List.map subst ts)
                          | _ -> t
                        in
                    ...
                    let has_tgen =
                      ...
                      let rec check t =
                        ...
                        | Types.TArrow (a, _, r) ->
                            check a;
                            check r
                        | Types.TTuple ts | Types.TVariant (_, ts) ->
                            List.iter check ts
                        | Types.TList t | Types.TArray t -> check t
                        | _ -> ()
                      in
                      List.iter check arg_types;
                      !found
                    in
                    if has_tgen then te
                    else begin
                ...
                | _ ->
                    ...
                    if
                      is_structural
                      && String.equal class_def.Types.class_name "Show"
                    then
                      let structural_ty =
                        List.find_map
                          (fun opt ->
                            ...
                            | Some ty -> (
                                match Types.repr ty with
                                | Types.TPolyVariant _ | Types.TRecord _ ->
                                    Some (Types.repr ty)
                                | _ -> None)
                            | None -> None)
                          partial_args
                      in
                      let show_ty =
                        match structural_ty with
                        | Some ty ->
                            Types.TArrow (ty, Types.EffEmpty, Types.TString)
                        | None -> Types.TUnit
                      in
                      mk_xf xctx
                        (TEField
                           ( mk_xf xctx
                               (TERecord
                                  [
                                    ( "show",
                                      mk_xf xctx (TEVar "__show_value") show_ty
                                    );
                                  ])
                               Types.TUnit,
                             method_name ))
...
and xform_constrained_ref xctx name te =
  ...
  | Some scheme when scheme_needs_xform scheme ->
      ...
      if scheme.Types.constraints <> [] then begin
        ...
        let dict_args =
          List.map
            (fun cc -> resolve_dict_arg xctx cc tgen_map)
            scheme.Types.constraints
        in
        result :=
          List.fold_left
            (fun fn dict_arg -> mk_xf xctx (TEApp (fn, dict_arg)) Types.TUnit)
            !result dict_args
      end;
      ...
      if scheme.Types.record_evidences <> [] then begin
        ...
        let ev_args =
          List.concat_map
            ...
            scheme.Types.record_evidences
        in
        result :=
          List.fold_left
            (fun fn ev_arg -> mk_xf xctx (TEApp (fn, ev_arg)) Types.TUnit)
            !result ev_args
      end;
      !result
  | _ -> te
...
and resolve_record_evidence_args xctx rgen_map (re : Types.record_evidence) =
      ...
      if is_closed arow then
        ...
        List.map
          (fun field_name ->
            let idx = ref (-1) in
            List.iteri
              (fun i n -> if String.equal n field_name then idx := i)
              sorted_names;
            if !idx >= 0 then mk_xf xctx (TEInt !idx) Types.TInt
            else
              error_xf xctx
                (Printf.sprintf
                   "field %s not found in concrete record for evidence"
                   field_name))
          re.re_fields
      else begin
            ...
            List.map
              (fun field_name ->
                match
                  List.find_map
                    (fun (rid, fname, pname) ->
                      if rid = id && String.equal fname field_name then
                        Some pname
                      else None)
                    xctx.xf_record_evidence
                with
                ...
                | None ->
                    error_xf xctx
                      (Printf.sprintf
                         "no evidence for field %s in polymorphic context"
                         field_name))
              re.re_fields
  ...
  | None ->
      error_xf xctx
        (Printf.sprintf "internal error: no record evidence for fields [%s]"
  ...
  | Some (class_name, method_name) -> (
      ...
      | Types.TVar { contents = Types.Unbound (id, _) } -> (
          ...
          | Some dparam ->
              ...
              let method_ref =
                mk_xf xctx (TEField (dict_ref, method_name)) Types.TUnit
              in
...
and xform_constrained_def xctx scheme te =
  ...
  let all_constrained, dict_param_names =
    List.fold_left
      (fun (entries, params) (cc : Types.class_constraint) ->
        ...
        | [] ->
           ...
           dict param to maintain positional alignment with call-site dict args. *)
            let dparam =
              Printf.sprintf "__dict_%s_resolved_%d" cc.cc_class
                (List.length params)
            in
            (entries, dparam :: params)
        | _ ->
            let dparam =
              Printf.sprintf "__dict_%s_%s" cc.cc_class
                (String.concat "_"
                   (List.map (fun (_, idx) -> string_of_int idx) tvar_entries))
            in
            (* All tvars for this constraint map to the same dict param *)
            let new_entries =
              List.map (fun (id, _) -> (id, dparam, cc.cc_class)) tvar_entries
            in
  ...
  let all_record_ev, ev_param_names =
    List.fold_left
      (fun (entries, params) (re : Types.record_evidence) ->
        List.fold_left
          (fun (entries, params) field_name ->
  ...
  List.fold_right
    (fun dparam body ->
      mk_xf xctx
        (TEFun (dparam, body, false))
        (Types.TArrow (Types.TUnit, Types.EffEmpty, body.ty)))
...
let xform_constrained_inst xctx inst_def dict_expr =
  let class_def =
    match
      Types.find_method_class xctx.xf_type_env.Types.classes
        (fst
           (List.hd
              ...
              | _ -> assert false)))
    with
    | Some cd when String.equal cd.Types.class_name inst_def.Types.inst_class ->
        cd
    | _ -> (
        (* Find class def directly *)
        match
          List.find_opt
            (fun c -> String.equal c.Types.class_name inst_def.Types.inst_class)
            xctx.xf_type_env.Types.classes
        with
        | Some cd -> cd
        | None ->
            error_xf xctx
              (Printf.sprintf "class %s not found" inst_def.inst_class))
  in
        ...
        (fun (mname, mte) ->
          ...
          | Some class_method_ty ->
              let rec walk_class s a =
                ...
                match s with
                | Types.TGen i ->
                    if not (Hashtbl.mem class_param_map i) then
        ...
        (fun class_idx stored_ty ->
          ...
          | Some actual_ty ->
              let rec walk_inst s a =
                ...
                match s with
                | Types.TGen i -> (
                    if not (Hashtbl.mem inst_tvar_map i) then
                      match a with
                      | Types.TVar { contents = Types.Unbound (id, _) } ->
    ...
    List.fold_left
      (fun (entries, params) (cc : Types.class_constraint) ->
        ...
        | _ ->
            ...
            in
            (new_entries @ entries, dparam :: params))
      ([], []) inst_def.inst_constraints
  in
  let dict_param_names = List.rev dict_param_names in
  let xctx =
    { xctx with xf_constrained = all_constrained @ xctx.xf_constrained }
  in
  ...
  List.iter
    (fun tdecl ->
      ...
      | TDLetRecAnd bindings ->
          List.iter
            (fun (_, te) -> improve_fundeps_in_expr vars type_env te)
            bindings
      | TDModule (_, decls, _) ->
          List.iter
            (fun d ->
    ...
    | tdecl :: rest ->
        let tdecl' =
          ...
          | TDModule (name, decls, mod_schemes) ->
              let mod_xctx =
                { xctx with xf_schemes = mod_schemes @ xctx.xf_schemes }
              in
...
let rec handler_body_walk ~pred (te : texpr) =
  ...
  | TEVar _ | TENil | TEContinueLoop ->
      false
  | TEFun _ -> false
  | TELetRec (_, _, _, e2) | TELetRecAnd (_, e2) -> handler_body_walk ~pred e2
  | TELet (_, _, e1, e2)
  | TESeq (e1, e2)
  | TELetMut (_, e1, e2)
  | TEBinop (_, e1, e2)
  ...
  | TEMatch (scrut, arms, _) ->
      ...
      || List.exists
           (fun (_, g, body) ->
             ...
             ||
             match g with
             | Some g -> handler_body_walk ~pred g
             | None -> false)
           arms
  | TEPerform (_, e)
  | TEUnop (_, e)
  ...
  | TEAssign (_, e) ->
      handler_body_walk ~pred e
  | TETuple es | TEArray es -> List.exists (handler_body_walk ~pred) es
  | TERecord fields ->
...
let handler_body_has_resume te =
  handler_body_walk
    ~pred:(fun te -> match te.expr with TEResume _ -> true | _ -> false)
    te

let handler_body_has_perform te =
  handler_body_walk
    ~pred:(fun te ->
      match te.expr with TEPerform _ | TEResume _ -> true | _ -> false)
    te
...
let rec classify_all_branches_resume (te : texpr) =
  ...
  | TELetMut (_, _, e2)
  | TESeq (_, e2) ->
      classify_all_branches_resume e2
...
let rec strip_tail_resume (te : texpr) =
  ...
  | TESeq (e1, e2) -> { te with expr = TESeq (e1, strip_tail_resume e2) }
  | TEIf (c, t, f) ->
      { te with expr = TEIf (c, strip_tail_resume t, strip_tail_resume f) }
  | TEMatch (s, arms, p) ->
      let arms' =
        List.map (fun (pat, g, body) -> (pat, g, strip_tail_resume body)) arms
      in
      ...
      let arms' =
        List.map
          (fun arm ->
            ...
            | THOp { op_name; arg; k; body } ->
                classify_arm
                  (THOp { op_name; arg; k; body = classify_texpr body })
