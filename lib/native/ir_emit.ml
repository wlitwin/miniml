...
type t = { buf : Buffer.t; mutable next_reg : int }

let create () = { buf = Buffer.create 4096; next_reg = 0 }
...
let emit_blank t = Buffer.add_char t.buf '\n'
...
let emit_label t name = Printf.bprintf t.buf "%s:\n" name

(* ---- Instructions ---- *)

let emit_ret t ty value = Printf.bprintf t.buf "  ret %s %s\n" ty value
let emit_ret_void t = Buffer.add_string t.buf "  ret void\n"
...
let emit_br t ~label = Printf.bprintf t.buf "  br label %%%s\n" label
...
let emit_phi t ~ty ~incoming =
  ...
  let pairs =
    String.concat ", "
      (List.map
         (fun (v, label) -> Printf.sprintf "[ %s, %%%s ]" v label)
         incoming)
  in
  Printf.bprintf t.buf "  %s = phi %s %s\n" reg ty pairs;
  reg

let emit_comment t msg = Printf.bprintf t.buf "  ; %s\n" msg
...
let emit_unreachable t = Buffer.add_string t.buf "  unreachable\n"
...
let emit_switch t ~value ~default ~cases =
  ...
  List.iter
    (fun (v, label) -> Printf.bprintf t.buf "    i64 %d, label %%%s\n" v label)
    cases;
