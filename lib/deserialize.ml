...
let tokenize src =
  ...
  while !i < len do
    ...
    | '-' | '0' .. '9' ->
        ...
        while !i < len && src.[!i] >= '0' && src.[!i] <= '9' do
          incr i
        done;
...
let json_field key = function
  | JObject fields -> (
      match List.assoc_opt key fields with Some v -> v | None -> JNull)
  | _ -> JNull
...
let deserialize_opcode json =
  ...
  | JArray (JString op :: args) -> (
      ...
      | "INDEX", [] -> INDEX
      | "HALT", [] -> HALT
      | "GET_LOCAL_CALL", [ JInt slot; JInt arity ] ->
          GET_LOCAL_CALL (slot, arity)
      | "GET_LOCAL_TUPLE_GET", [ JInt slot; JInt idx ] ->
          GET_LOCAL_TUPLE_GET (slot, idx)
      | "GET_LOCAL_FIELD", [ JInt slot; JString name ] ->
          GET_LOCAL_FIELD (slot, name)
      | "GET_GLOBAL_CALL", [ JInt idx; JInt arity ] ->
          GET_GLOBAL_CALL (idx, arity)
      | "GET_GLOBAL_FIELD", [ JInt idx; JString name ] ->
          GET_GLOBAL_FIELD (idx, name)
      | "JUMP_TABLE", [ JInt min_tag; JArray targets; JInt default ] ->
          JUMP_TABLE
            (min_tag, Array.of_list (List.map json_int targets), default)
...
let rec deserialize_value json =
  ...
  | "T" ->
      Bytecode.VTuple
        (Array.of_list
           (List.map deserialize_value (json_list (json_field "v" json))))
  | "L" ->
      Bytecode.VList
        (List.map deserialize_value (json_list (json_field "v" json)))
  | "V" ->
      ...
      let payload =
        match json_field "payload" json with
        | JNull -> None
...
and deserialize_prototype json =
  ...
  let code =
    Array.of_list
      (List.map deserialize_opcode (json_list (json_field "code" json)))
  in
  let constants =
    Array.of_list
      (List.map deserialize_value (json_list (json_field "constants" json)))
  in
  let line_table =
    Array.of_list (List.map json_int (json_list (json_field "line_table" json)))
  in
                    ...
                    (Bytecode.VExternal
                       {
                         ...
                         ext_fn =
                           (fun _ ->
                             raise
                               (Vm.Runtime_error
                                  (Printf.sprintf "unregistered builtin: %s"
                                     name)));
                         ext_args = [];
                       }))
          | "dict" ->
              ...
              let field_list =
                match fields_json with JObject fl -> fl | _ -> []
              in
              let methods =
                List.map
              ...
              Hashtbl.replace globals idx
                (Bytecode.make_record field_names values)
          | _ -> ())
        fields
  | _ -> ());
