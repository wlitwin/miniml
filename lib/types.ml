...
type class_arg =
  ...
  | CAWild
    (* fundep-determined: not in method type, resolved via fundep at call site *)
  | CAPhantom of
      int
      * ty (* fundep-determined from annotation: unique index + actual tvar *)

type class_constraint = {
  cc_class : string; (* class name, e.g. "Show" *)
  cc_args : class_arg list;
      (* one per class param: TGen index or concrete type *)
}
...
type scheme = {
  ...
  record_evidences : record_evidence list;
      (* field offset evidence for row-polymorphic updates *)
...
let new_tvar level = TVar (ref (Unbound (fresh_id (), level)))
let new_effvar level = EffVar (ref (EffUnbound (fresh_id (), level)))
let new_pvvar level = PVVar (ref (PVUnbound (fresh_id (), level)))
let new_rvar level = RVar (ref (RUnbound (fresh_id (), level)))
...
let rec is_concrete ty =
  ...
  | TArrow (a, eff, r) | TCont (a, eff, r) ->
      is_concrete a && is_concrete_eff eff && is_concrete r
...
and is_concrete_eff = function
  ...
  | EffRow (_, tys, tail) ->
      List.for_all is_concrete tys && is_concrete_eff tail
  | EffEmpty -> true
  | EffGen _ -> true
...
type ctor_info = {
  ...
  ctor_return_ty_params : ty list option;
      (* None = ADT, Some = GADT return type params *)
...
type class_def = {
  ...
  class_methods : (string * ty) list;
      (* types use TGen 0..N-1 for class params *)
...
type effect_def = {
  ...
  effect_params : string list;
      (* type parameter names, e.g. ["a"] for State 'a *)
...
type type_env = {
  ...
  newtypes : string list;
      (* type names whose single constructor is erased at runtime *)
  hidden_types : string list;
      (* module-private type names not accessible from outer scope *)
  hidden_ctor_types : string list;
      (* type names whose constructors are hidden (private + opaque) *)
}
...
and occurs_check_eff_for_ty id level eff =
  ...
  | _ -> ()
(* eff_repr resolved links; only EffVar(Unbound), EffEmpty, EffGen remain *)
    ...
    List.find_map
      (fun (name, num_params, pattern) ->
        ...
        let rec go pat actual =
          ...
          | TCont (p1, _pe, p2), TCont (a1, _ae, a2) ->
              go p1 a1 && go p2 a2
...
and pp_ty_impl synonyms ty =
  ...
  | Some (name, args) -> (
      ...
      match ty with
      | (TArrow (a, _, _) | TCont (a, _, _))
        when try_match_synonym synonyms (repr a) <> None ->
...
and pp_type_con_impl synonyms name = function
  ...
  | [ arg ] ->
      (match repr arg with
        ...
        | _ -> pp_ty_impl synonyms arg)
      ^ " " ^ name
  | args ->
      "("
      ^ String.concat ", " (List.map (pp_ty_impl synonyms) args)
      ^ ") " ^ name
...
and needs_parens_impl synonyms ty =
  ...
  | Some _ -> false
  | None -> (
      match ty with TArrow _ | TCont _ | TTuple _ -> true | _ -> false)

and pp_ty_arg_impl synonyms ty =
  if needs_parens_impl synonyms ty then "(" ^ pp_ty_impl synonyms ty ^ ")"
  else pp_ty_impl synonyms ty
...
and pp_ty_raw_impl synonyms ty =
  ...
  | TArrow (a, eff, r) ->
      ...
      in
      if eff_is_trivial eff then a_str ^ " -> " ^ pp_ty_impl synonyms r
      else
        a_str ^ " -> " ^ pp_ty_impl synonyms r ^ " / "
        ^ pp_eff_impl synonyms eff
  | TCont (a, eff, r) ->
      let inner =
        let a_str =
          match repr a with
      ...
      String.concat " * "
        (List.map
           (fun t ->
             match repr t with
             | (TArrow _ | TCont _)
               when try_match_synonym synonyms (repr t) = None ->
  ...
  | TRecord row ->
      ...
      let fields_str =
        String.concat "; "
          (List.map
             (fun (n, t) -> n ^ ": " ^ pp_ty_impl synonyms t)
             (collect row))
      in
      if is_open row then
...
let rec collect_eff_row eff =
  match eff_repr eff with
  | EffRow (l, p, tail) ->
      let rest, ft = collect_eff_row tail in
      ((l, p) :: rest, ft)
  | other -> ([], other)
...
let rec collect_pv_row row =
  match pv_repr row with
  | PVRow (t, ty, tail) ->
      let rest, ft = collect_pv_row tail in
      ((t, ty) :: rest, ft)
  | other -> ([], other)
...
let rec collect_rrow row =
  match rrow_repr row with
  | RRow (f, ty, tail) ->
      let rest, ft = collect_rrow tail in
      ((f, ty) :: rest, ft)
  | other -> ([], other)
...
let rec unify_eff e1 e2 =
  ...
  if e1 == e2 then ()
    ...
    | EffRow (_label1, _params1, _tail1), _ ->
        ...
        let shared =
          match (ft1, ft2) with EffVar r1, EffVar r2 -> r1 == r2 | _ -> false
        in
        if shared then unify_eff_shared_tail e1 e2
        else
          let label1, params1, tail1 =
            match e1 with EffRow (l, p, t) -> (l, p, t) | _ -> assert false
          in
    ...
    | _ ->
        raise
          (Unify_error
             (Printf.sprintf "cannot unify effects %s and %s" (pp_eff e1)
                (pp_eff e2)))
...
and rewrite_row label params_to_unify eff =
  ...
  | _ ->
      raise
        (Unify_error (Printf.sprintf "cannot rewrite effect row for %s" label))
...
and unify_eff_shared_tail e1 e2 =
  ...
  let level =
    ...
    | EffVar { contents = EffUnbound (_, l) } -> l
    | _ -> assert false
  in
  ...
  List.iter
    (fun (l1, p1) ->
      let rec find_and_remove = function
        | [] -> (None, [])
        | (l2, p2) :: rest when String.equal l1 l2 -> (Some p2, rest)
        | x :: rest ->
      ...
      match found with
      | Some p2 ->
          if List.length p1 = List.length p2 then List.iter2 unify p1 p2
          else
            raise
              (Unify_error
                 (Printf.sprintf "cannot unify effects %s and %s" (pp_eff e1)
  ...
  let new_tail =
    List.fold_right
      (fun (l, p) acc -> EffRow (l, p, acc))
      (List.rev !only1 @ !remaining2)
      fresh
  in
...
and subeffect source target =
  ...
  if source == target then ()
    ...
    | EffRow (_label, _params, _tail) ->
        ...
        let shared =
          match (ft_source, ft_target) with
          | EffVar r1, EffVar r2 -> r1 == r2
          | _ -> false
        in
        if shared then unify_eff_shared_tail source target
        else
          let label, params, tail =
            match source with
            | EffRow (l, p, t) -> (l, p, t)
            | _ -> assert false
          in
...
and unify_pv_row r1 r2 =
  ...
  if r1 == r2 then ()
    ...
    | row, PVVar ({ contents = PVUnbound (id, level) } as r) ->
        ...
        r := PVLink row
    | PVEmpty, PVEmpty -> ()
    | PVRow (tag1, ty1, tail1), PVRow (tag2, ty2, tail2)
    ...
    | PVRow (_tag1, _ty1, _tail1), _ ->
        ...
        let shared =
          match (ft1, ft2) with
          | PVVar rv1, PVVar rv2 -> rv1 == rv2
          | _ -> false
        in
        if shared then unify_pv_shared_tail r1 r2
        else
          let tag1, ty1, tail1 =
            match r1 with PVRow (t, ty, tl) -> (t, ty, tl) | _ -> assert false
          in
    ...
    | _ -> raise (Unify_error "cannot unify polymorphic variant types")
...
and rewrite_pv_row tag default_payload row =
  ...
  | PVEmpty ->
      raise
        (Unify_error
           (Printf.sprintf "tag `%s not in closed polymorphic variant type" tag))
  | _ ->
      raise
        (Unify_error
           (Printf.sprintf "cannot rewrite polymorphic variant row for `%s" tag))
...
and unify_pv_shared_tail r1 r2 =
  ...
  let level =
    ...
    | PVVar { contents = PVUnbound (_, l) } -> l
    | _ -> assert false
  in
  ...
  List.iter
    (fun (t1, ty1) ->
      let rec find_and_remove = function
        ...
        | x :: rest ->
            let f, r = find_and_remove rest in
            (f, x :: r)
      in
      let found, rest = find_and_remove !remaining2 in
      remaining2 := rest;
...
and unify_record_row r1 r2 =
  ...
  if r1 == r2 then ()
    ...
    | RRow (_f1, _ty1, _tail1), _ ->
        ...
        let shared =
          match (ft1, ft2) with RVar rv1, RVar rv2 -> rv1 == rv2 | _ -> false
        in
        if shared then unify_rrow_shared_tail r1 r2
        else
          let f1, ty1, tail1 =
            match r1 with RRow (f, t, tl) -> (f, t, tl) | _ -> assert false
          in
...
and rewrite_record_row field row =
  ...
  | RWild ->
      ...
      (field_ty, RWild)
  | REmpty ->
      raise (Unify_error (Printf.sprintf "record has no field %s" field))
  | _ ->
      raise
        (Unify_error (Printf.sprintf "cannot rewrite record row for %s" field))
  ...
  and go_eff eff =
    ...
    | EffVar { contents = EffUnbound (id, l) } when l > level -> (
        ...
        | None ->
            ...
            gen)
    | EffVar { contents = EffUnbound _ } | EffVar { contents = EffLink _ } ->
        eff
    | EffRow (label, params, tail) ->
...
let pp_constraint_impl synonyms (c : class_constraint) =
  c.cc_class ^ " "
  ^ String.concat " " (List.map (pp_class_arg_impl synonyms) c.cc_args)
...
let pp_scheme_impl synonyms (s : scheme) =
  ...
  let rec collect ty =
    ...
    | TGen i ->
        if not (Hashtbl.mem remap i) then begin
          Hashtbl.replace remap i !counter;
          incr counter
        end
    | TArrow (a, _, r) | TCont (a, _, r) ->
        collect a;
        collect r
  ...
  and collect_pvrow row =
    match row with
    | PVRow (_, ty_opt, tail) ->
        Option.iter collect ty_opt;
        collect_pvrow tail
    | _ -> ()
  in
  collect s.body;
  let rename_tgen i =
    match Hashtbl.find_opt remap i with Some j -> j | None -> i
  in
  let rec rewrite ty =
    match ty with
  ...
  and rewrite_pvrow row =
    match row with
    | PVRow (tag, ty_opt, tail) ->
        PVRow (tag, Option.map rewrite ty_opt, rewrite_pvrow tail)
    | r -> r
  in
  let rename_arg = function CATGen i -> CATGen (rename_tgen i) | a -> a in
  ...
  | cs ->
      let cs =
        List.map
          (fun (c : class_constraint) ->
            { c with cc_args = List.map rename_arg c.cc_args })
          cs
      in
      let cs_str =
        String.concat ", " (List.map (pp_constraint_impl synonyms) cs)
      in
...
let instantiate_with_mapping level (s : scheme) =
  if s.quant = 0 && s.equant = 0 && s.pvquant = 0 && s.rquant = 0 then
    ...
    and go_pv = function
      ...
      | PVRow (tag, ty_opt, tail) ->
          PVRow (tag, Option.map go ty_opt, go_pv tail)
      | r -> r
...
and deep_repr_eff eff =
  match eff_repr eff with
  | EffRow (label, params, tail) ->
      EffRow (label, List.map deep_repr params, deep_repr_eff tail)
  | e -> e

and deep_repr_pv row =
  match pv_repr row with
  | PVRow (tag, ty_opt, tail) ->
      PVRow (tag, Option.map deep_repr ty_opt, deep_repr_pv tail)
  | r -> r
...
let rec types_compatible t1 t2 =
  ...
  | TTuple ts1, TTuple ts2 ->
      ...
      && List.for_all2 types_compatible ts1 ts2
  ...
  | TRecord r1, TRecord r2 ->
      ...
      && List.for_all2
           (fun (n1, t1) (n2, t2) ->
             String.equal n1 n2 && types_compatible t1 t2)
           f1 f2
...
let rec subtype t1 t2 =
  ...
  | TTuple ts1, TTuple ts2 ->
      List.length ts1 = List.length ts2 && List.for_all2 subtype ts1 ts2
  | TVar r1, TVar r2 when r1 == r2 -> true
  | _ -> false
...
let rec equal_ty t1 t2 =
  ...
  | TTuple ts1, TTuple ts2 ->
      List.length ts1 = List.length ts2 && List.for_all2 equal_ty ts1 ts2
...
let freshen_unbound ty =
  ...
  and go_rrow = function
    ...
    | r -> r
  and go_pv = function
    | PVRow (tag, payload, tail) ->
        PVRow (tag, Option.map go payload, go_pv tail)
...
let match_partial_inst inst_tys partial =
  ...
  let rec match_one inst conc =
    ...
    | TGen i -> (
        ...
        | None ->
            Hashtbl.replace bindings i conc;
            true)
    ...
    | TArray a -> (
        match repr conc with TArray b -> match_one a b | _ -> false)
    | TTuple ts1 -> (
    ...
    | TRecord r1 -> (
        ...
        | TRecord r2 ->
            ...
            && List.for_all2
                 (fun (n1, t1) (n2, t2) ->
                   String.equal n1 n2 && match_one t1 t2)
                 f1 f2
        | _ -> false)
    | TVariant (a, args_inst) -> (
        match repr conc with
        | TVariant (b, args_conc)
          when String.equal a b && List.length args_inst = List.length args_conc
          ->
...
let inst_specificity (inst : instance_def) =
  let rec count = function
    ...
    | TRecord row ->
        List.fold_left
          (fun acc (_, t) -> acc + count t)
          0 (record_row_to_fields row)
    ...
    (fun fd ->
      List.iter
        (fun existing ->
          ...
          else if List.length existing.inst_tys <> List.length new_inst.inst_tys
          then ()
          else begin
            ...
            if from_match then begin
              ...
              if not to_match then
                let from_strs =
                  List.map
                    (fun i -> pp_ty (List.nth new_inst.inst_tys i))
                    fd.fd_from
                in
                let to_strs_new =
                  List.map
                ...
                let to_strs_ex =
                  List.map
                    (fun i -> pp_ty (List.nth existing.inst_tys i))
                    fd.fd_to
                in
                raise
                  (Unify_error
                     (Printf.sprintf
                        "functional dependency violation in class %s: type(s) \
                         %s determine %s, but existing instance has %s"
...
let improve_with_fundeps instances class_def partial =
  List.fold_left
    (fun partial fd ->
      ...
      let from_known =
        List.for_all
          (fun i -> i < List.length partial && List.nth partial i <> None)
          fd.fd_from
      in
      if not from_known then partial
        ...
        let from_partial =
          List.mapi
            (fun i opt -> if List.mem i fd.fd_from then opt else None)
            partial
        in
        ...
        | [ inst ] ->
            ...
            List.iter
              (fun i ->
                ...
                let rec extract_bindings pat conc =
                  ...
                  | TVariant (_, args1), TVariant (_, args2)
            ...
            let rec apply_subst ty =
              match ty with
              | TGen g -> (
                  match Hashtbl.find_opt subst g with Some t -> t | None -> ty)
