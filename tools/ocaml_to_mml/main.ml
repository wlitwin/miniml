...
let translate_module_name name =
  match name with "Hashtbl" -> "Hashtbl" | _ -> name
...
let is_prefix_op op =
  match op with "!" | "~-" | "~-." | "not" | "lnot" -> true | _ -> false
...
let rec emit_type e (ct : core_type) =
  ...
  | Ptyp_arrow (_, t1, t2) ->
      let needs_parens =
        match t1.ptyp_desc with Ptyp_arrow _ -> true | _ -> false
      in
      ...
      List.iteri
        (fun i t ->
          ...
          let needs_parens =
            ...
            | Ptyp_arrow _ | Ptyp_tuple _ -> true
            | _ -> false
          in
          if needs_parens then emit e "(";
          emit_type e t;
      ...
      if name = "ref" then begin
        ...
        | [ arg ] ->
            let needs_parens =
              ...
              | Ptyp_tuple _ | Ptyp_arrow _ -> true
              | _ -> false
            in
            if needs_parens then emit e "(";
            emit_type e arg;
  ...
  | Ptyp_alias (t, s) ->
      emit_type e t;
      emit e (" as '" ^ s.txt)
  | Ptyp_variant _ -> emit e "(* TODO: polymorphic variant type *)"
  | Ptyp_poly (_, t) -> emit_type e t
          ...
          Option.iter
            (fun (_, p) ->
              ...
              let needs_parens =
                ...
                | Ppat_tuple _ | Ppat_construct (_, Some _) | Ppat_alias _ ->
                    true
                | _ -> false
              in
              if needs_parens then emit e "(";
...
and emit_constant e (c : Parsetree.constant) =
  ...
  | Pconst_integer (s, _) ->
      if String.length s > 0 && s.[0] = '-' then emit e ("(" ^ s ^ ")")
      else emit e s
  | Pconst_float (s, _) ->
      if String.length s > 0 && s.[0] = '-' then emit e ("(" ^ s ^ ")")
      else emit e s
...
and is_atom (expr : expression) =
  ...
  | Pexp_ident _ | Pexp_constant _ | Pexp_tuple _ | Pexp_array _ | Pexp_record _
...
and emit_expr e (expr : expression) =
  ...
  | Pexp_apply (fn, args) -> emit_apply e fn args
      ...
      List.iteri
        (fun i (lid, ex) ->
          ...
          | Pexp_ident { txt = Longident.Lident n; _ }
            when String.equal (escape_keyword n) fname ->
              emit e fname
          | _ ->
              emit e fname;
              emit e " = ";
  ...
  | Pexp_for (var, lo, hi, dir, body) ->
      let var_name =
        match var.ppat_desc with Ppat_var v -> v.txt | _ -> "_loop_var"
      in
      emit e "let mut ";
      emit e var_name;
      emit e " = ";
      ...
      else emit_expr e body;
      emit e ";";
      newline e;
      emit e var_name;
      emit e
        (if dir = Upto then " := " ^ var_name ^ " + 1"
         else " := " ^ var_name ^ " - 1");
      newline e;
      dedent e;
      emit e "end"
  ...
  List.iter
    (fun (p : function_param) ->
      ...
      | Pparam_val (_lbl, _default, pat) ->
          emit e " ";
          emit_pattern e pat
      | Pparam_newtype name -> emit e (" (type " ^ name.txt ^ ")"))
    params;
...
and needs_parens_subexpr (expr : expression) =
  match expr.pexp_desc with
  | Pexp_apply _ | Pexp_match _ | Pexp_ifthenelse _ | Pexp_let _
  | Pexp_function _ | Pexp_sequence _
...
and emit_apply e fn args =
  ...
  | ( Pexp_ident { txt = Lident "raise"; _ },
      [
        ( _,
          { pexp_desc = Pexp_construct ({ txt = Lident ctor; _ }, payload); _ }
        );
      ] ) -> (
  ...
  | _ ->
      let needs_parens_fn =
        ...
        | Pexp_function _ | Pexp_match _ | Pexp_ifthenelse _ | Pexp_let _ ->
            true
        | _ -> false
      in
  ...
  List.iter
    (fun (case : case) ->
      ...
      let rec rewrite_char_pat (pat : pattern) : pattern =
        match pat.ppat_desc with
        | Ppat_constant c
          when match c.pconst_desc with Pconst_char _ -> true | _ -> false ->
            let var_name =
              if !char_counter = 0 then "_c"
              else Printf.sprintf "_c%d" !char_counter
            in
            incr char_counter;
        ...
        | Ppat_construct (lid, Some (vars, inner)) ->
            {
              pat with
              ppat_desc =
                Ppat_construct (lid, Some (vars, rewrite_char_pat inner));
            }
        | Ppat_or (p1, p2) ->
            {
              pat with
              ppat_desc = Ppat_or (rewrite_char_pat p1, rewrite_char_pat p2);
            }
        | _ -> pat
      in
      ...
      let is_multiline =
        ...
        | Pexp_let _ | Pexp_sequence _ | Pexp_match _ | Pexp_ifthenelse _ ->
            true
        | _ -> false
      in
...
and decompose_fun_binding (vb : value_binding) =
  ...
  | Pexp_function (params, constraint_, body) -> (
      ...
      in
      let ret_type =
        match constraint_ with Some (Pconstraint ty) -> Some ty | _ -> None
      in
      match body with
      | Pfunction_body e -> (name, pats, ret_type, e)
      | Pfunction_cases (cases, _, _) ->
          let fn_expr =
            {
              vb.pvb_expr with
              pexp_desc =
                Pexp_function
                  ([], None, Pfunction_cases (cases, Location.none, []));
            }
          in
          (name, pats, ret_type, fn_expr))
  | Pexp_constraint (inner, ty) -> (name, [], Some ty, inner)
...
let rec emit_structure_item e (item : structure_item) =
  ...
  | Pstr_module mb ->
      ...
      | Pmod_structure items ->
          List.iter (fun item -> emit_structure_item e item) items
...
let translate_file filename =
  ...
  let opens =
    match String.lowercase_ascii modname with
    | "lexer" | "parser" -> [ "Token" ]
...
let () =
  ...
  | [] ->
      ...
      exit 1
  | files -> List.iter translate_file files
