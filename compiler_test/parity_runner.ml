...
(* The .tests format parser lives in Test_format (cross_test/test_format.ml)
   — the single OCaml implementation shared by all runners. *)
...
let is_selfhost_type_error msg =
  contains_substring msg "Type error"
  || contains_substring msg "type error"
  || contains_substring msg "TypeError"
  || contains_substring msg "Unify error"
  || contains_substring msg "unbound variable"
  || contains_substring msg "unknown type"
  ...
  || contains_substring msg "expects"
  ...
  || contains_substring msg "ambiguous instance"
...
let batch_selfhost_compile ~output_fn ~argv prepared tests =
  ...
  if not batch_ok then begin
    let last_output =
      ...
      | parts -> (
          ...
          | last :: _ ->
              ...
              ^ String.sub last 0 (min 200 (String.length last)))
    in
    Printf.eprintf
      "\n\
    ...
    Array.make (List.length tests)
      (CompileErr ("batch compilation failed: " ^ !batch_err))
  end
  else begin
  ...
  List.iter
    (fun tc ->
      ...
      | Value expected, Ok ocaml_out, Ok selfhost_out ->
          ...
          else if ocaml_out <> expected then
            fail tc.name "OCaml compiler wrong: expected %S, got %S" expected
              ocaml_out
          else
            fail tc.name "parity mismatch: ocaml=%S, selfhost=%S" ocaml_out
              selfhost_out
      ...
      | TypeErrorMsg substr, TypeErr ocaml_msg, TypeErr _selfhost_msg ->
          if contains_substring ocaml_msg substr then begin
            ...
            fail tc.name "OCaml type error doesn't contain %S: %s" substr
              ocaml_msg
      ...
      | RuntimeError substr, RuntimeErr ocaml_msg, RuntimeErr _selfhost_msg ->
          if contains_substring ocaml_msg substr then begin
            ...
            fail tc.name "OCaml runtime error doesn't contain %S: %s" substr
              ocaml_msg
      (* Parity violations *)
...
let () =
  ...
  let self_host_files =
    [
      "self_host/token.mml";
      "self_host/ast.mml";
      "self_host/bytecode.mml";
      "self_host/types.mml";
      "self_host/lexer.mml";
      "self_host/parser.mml";
      "self_host/typechecker.mml";
      "self_host/compiler.mml";
      "self_host/serialize.mml";
      "self_host/main.mml";
    ]
  in
  let compiler_mtime =
    try (Unix.stat compiler_json_path).Unix.st_mtime with _ -> 0.0
  in
  let stale_files =
    List.filter
      (fun f ->
        try (Unix.stat f).Unix.st_mtime > compiler_mtime with _ -> false)
      self_host_files
  in
  if stale_files <> [] then
    Printf.eprintf
      "WARNING: %s is older than: %s\n\
      \  Run 'make self-host-compile' to rebuild.\n\
       %!"
      compiler_json_path
      (String.concat ", " stale_files);
  ...
  let prepared =
    Interpreter.Deserialize.prepare_bundle compiler_parsed builtins
  in
  ...
  let files =
    if arg_files <> [] then arg_files
      ...
      with Unix.Unix_error _ -> (
        ...
        with Unix.Unix_error _ ->
          Printf.eprintf
            "No test files found. Usage: parity_runner [file.tests ...] [-t \
             \"name\" ...]\n";
          exit 1)
  in
  ...
  let file_tests =
    List.map
      (fun file -> (file, filter_tests filters (parse_test_file file)))
      files
  in
  ...
  let all_compiled =
    batch_selfhost_compile ~output_fn:init_state.Interpreter.Interp.output_fn
  ...
  List.iter
    (fun (file, tests) ->
      ...
      if tests = [] && filters <> [] then
        ...
        let p, f, failures =
          run_tests ~output_fn:init_state.Interpreter.Interp.output_fn state
            builtins tests file_compiled
        in
    ...
    file_tests;
  Printf.printf "==============================\n";
  Printf.printf "%d/%d parity tests passed" !total_passed
